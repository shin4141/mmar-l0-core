...
def _pure_turn_fallback(cfg: DebateConfig, speaker: str, turn_no: int, latest_opponent: str = "", transcript: str = "", own_previous: str = "") -> str:
    ...
    if turn_no == 2:
        ...
        analogy_line = _turn2_translation_analogy(f"{core} {own_reason}") if core else ""
        return _sanitize_fighter_speech(
            f"{weakness} {own_reason}{(' ' + analogy_line) if analogy_line else ''}"
        )
    position = cfg.side_a if speaker == "A" else cfg.side_b
    short_clause = _turn2_opponent_short_clause(latest_opponent) or "その押し返し"
    topic = _clean_text(cfg.topic or "")
    if "AI" in topic and "意識" in topic:
        gap = "主観的経験の有無"
    elif "AI" in topic and "得する人" in topic and "損する人" in topic:
        gap = "再配置の遅れ"
    elif "金" in topic and "銀" in topic:
        gap = "長期保有の条件差"
    else:
        gap = "決め手の不足"
    return _sanitize_fighter_speech(
        f"相手の主張は「{short_clause}」だが、{gap}は残る。 したがって、{_turn1_direct_claim(cfg.topic, position).rstrip('。')}。"
    )
...
def _to_casual_pure(turn_no: int, text: str) -> str:
    ...
    for sentence in sentences:
        ...
        short = short.replace("その「", "その")
        short = short.replace("」だけでは、", "だけでは、")
        short = short.replace("そのその一点", "その一点")
        short = short.replace("、まだ", "、")
        short = re.sub(r"^相手の主張は", "相手は", short)
...
def _turn1_answer_prompt(topic: str, position: str, seed: str = "") -> str:
    ...
    return (
        ...
        "\n"
        "Write the strongest argument in Japanese.\n"
        "\n"
        "- Start with a clear conclusion.\n"
        "- Add one causal explanation.\n"
...
def _turn1_direct_support_reason(topic: str, position: str) -> str:
    ...
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        if negative:
            return "因果: 会話の流暢さが上がっても、主観的な感覚と自発的な欲求が示せなければ、意識があるとは言えない。"
        return "因果: 自己参照と長い対話の一貫性が積み上がるほど、意識を完全にゼロと断定する根拠の方が弱くなる。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        if negative:
            return "因果: 自動化が先に仕事を削り、再配置と再訓練が遅れると、得より先に損を受ける人が広がる。"
        return "因果: 自動化で単純作業の負担が減り、再配置と新しい需要が広がるほど、得を受ける人の方が増える。"
    if "SaaS" in cleaned_topic and "終わり" in cleaned_topic:
        if any(token in cleaned_position for token in ["終わっていない", "終わりではない"]):
            return "因果: 継続率が残る領域では値上げと利用定着が効き、解約率が低い企業から先に収益が粘る。"
        return "因果: 金利上昇でCACが悪化し、解約率が上がると、LTV/CACが崩れた企業から先に失速する。"
    if "金" in cleaned_topic and "銀" in cleaned_topic:
        if "銀" in cleaned_position:
            return "因果: 景気減速でも実需の下支えが残る局面では、銀は金ほど一方向に偏らず値持ちしやすい。"
        return "因果: 景気減速で産業需要が落ちると、銀は金より値動きが荒くなり、長く持つほど不利が出やすい。"
    ...
    return f"因果: {first}が崩れると{second}も崩れ、その差が結論を分ける。"
...
def _turn1_direct_example(topic: str, position: str) -> str:
    ...
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        if negative:
            return "具体: 大規模言語モデルは自然に対話しても、痛みや欲求を自分で感じた観測証拠はまだ示していない。"
        return "具体: 長い対話で自己参照や内省を一貫して返すAIは増えており、意識の境界は単純に切れなくなっている。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        if negative:
    ...
    return f"具体: {first}をめぐる現場では、{second}の差が先に数字として表れる。"
...
def _turn1_direct_surface(cfg: DebateConfig, speaker: str) -> str:
    ...
    return _sanitize_fighter_speech(
        f"{proposition} 理由は、{support_reason} {example_line} {preemption}"
    )
...
def _turn2_attack_line(cfg: DebateConfig, speaker: str, latest_opponent: str, core: str) -> str:
    ...
    if "存在" in topic:
        if "存在しない" in opponent or "いない" in opponent:
            return "未検出をそのまま不在の証明にしているのが弱い。"
        if "存在する" in opponent or "いる" in opponent:
...
def _turn2_own_reinforcement(cfg: DebateConfig, speaker: str, own_previous: str = "") -> str:
    ...
    if "存在" in topic:
        if speaker == "A":
            return "見つかっていない以上、観測空白は存在肯定の根拠にならない。"
        return "探索が足りない以上、未検出だけで不在とは言えない。"
    support = ""
