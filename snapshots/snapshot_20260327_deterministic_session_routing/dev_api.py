...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path not in {"/api/debate", "/api/ask_match", "/api/history/save", "/api/provider_preflight", "/api/judge"} and not path.startswith("/api/history/view/") and not path.startswith("/api/history/like/"):
            self._send_json(404, {"ok": False, "error": "not found"})
            return
