...
def _short_stance_opening_surface(
    ...
    if speaker == "A":
        if lane == "science":
            return _sanitize_fighter_speech(
                f"{proposition}"
                f" 理由は、{primary_term}の観測や{secondary_term}の探索範囲を見ても、再現可能な証拠がまだ決め手になっていないからだ。"
                f" だから{own_line}"
            )
        if lane == "product":
            return _sanitize_fighter_speech(
                f"{proposition}"
                f" 理由は、{primary_term}や{secondary_term}のような現実条件が、その結論をまだ支えているからだ。"
                f" 例えば{concrete}を見ると、急いで逆転したとは言えない。"
            )
        return _sanitize_fighter_speech(
            f"{proposition}"
            f" 理由は、{primary_term}や{secondary_term}の現実条件が、この立場をまだ支えているからだ。"
            f" だから{own_line}"
        )
    if lane == "science":
        return _sanitize_fighter_speech(
            f"{proposition}"
            f" 理由は、{primary_term}の観測空白や{secondary_term}の探索限界を見ると、その可能性をまだ切れないからだ。"
            f" 例えば観測空白が残る限り、存在をゼロとは断定できない。"
        )
    if lane == "product":
        return _sanitize_fighter_speech(
            f"{proposition}"
            f" 理由は、{primary_term}や{secondary_term}が今も収益や継続利用を支え、その命題をまだ否定できないからだ。"
            f" 例えばvertical SaaSや高い切替コストが残る市場では、同じ型がまだ続く。"
        )
    return _sanitize_fighter_speech(
        f"{proposition}"
        f" 理由は、{primary_term}や{secondary_term}を見れば、この命題をまだ切れないからだ。"
        f" だから{own_line}"
    )
...
def _three_turn_validation_report(
    ...
    debug["three_turn_contract_pass"] = debug["turn_role_complete"] and not any(item in fatal_failures for item in failures)
