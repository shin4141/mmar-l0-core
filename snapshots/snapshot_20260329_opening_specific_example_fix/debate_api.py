...
def _short_stance_opening_surface(
    ...
    if lane == "science":
        ...
        support_reason = f"{primary_term}が欠けると{secondary_term}を広げても結論は立たず、主張の骨がそこで止まるからだ。"
        attack_preempt = f"{secondary_term}の広さだけで押す見方は、{primary_term}が無いという穴を埋めていない。"
    elif lane == "product":
        primary_term = lane_terms[0] if lane_terms else "市場の継続需要"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "切替コスト"
        support_reason = f"{primary_term}が崩れると{secondary_term}まで細り、成長の前提そのものが連鎖で弱るからだ。"
        attack_preempt = f"追い風だけで押す見方は、{primary_term}と{secondary_term}の連鎖を見落としている。"
    elif lane == "social":
        primary_term = lane_terms[0] if lane_terms else "制度運用"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "実装条件"
        support_reason = f"{primary_term}が止まると{secondary_term}も詰まり、制度全体がそのまま止まるからだ。"
        attack_preempt = f"理念や単発例外だけでは、{primary_term}と{secondary_term}の運用条件は崩せない。"
    else:
        primary_term = lane_terms[0] if lane_terms else "現実条件"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "採用条件"
        support_reason = f"{primary_term}から{secondary_term}まで因果がつながるときだけ、この立場は現実に残るからだ。"
        attack_preempt = f"抽象論だけで押す見方は、{primary_term}と{secondary_term}の接続を示せていない。"
    if speaker == "A":
        return _sanitize_fighter_speech(
            f"{proposition}"
