...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig) -> tuple[dict[str, Any], dict[str, dict[str, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        if cfg.turn_count == 3 and _should_force_short_b_rebuttal_repair(cfg, "B", turn_no, b_validation):
            repaired = _short_b_turn2_repair(cfg, _three_turn_resolution_line(cfg.side_b, "B", _build_proposition_lock(cfg)), _three_turn_issue_anchor(cfg, 0), _build_proposition_lock(cfg))
            repaired_validation = _three_turn_validation_report("B", cfg, turns, turn_no, repaired, b_latest_opponent)
            if repaired_validation.get("three_turn_contract_pass"):
                b_text = repaired
                b_validation = repaired_validation
                b_debug_trace["repair_triggered"] = True
                b_debug_trace["repair_text"] = b_text
                b_debug_trace["repair_validation"] = dict(b_validation)
                b_debug_trace["adopted_stage"] = "short-b-repair"
...
def _short_b_turn2_repair(cfg: DebateConfig, own_line: str, issue_primary: str, proposition_lock: dict[str, Any]) -> str:
    return _sanitize_fighter_speech(
        f"相手の核は{issue_primary}だ。"
        f" だが{_three_turn_concrete_support('B', proposition_lock, 2)}"
        " 中央銀行の決済・担保・保管の実務でその不安定さが残る以上、相手の前提は成立しない。"
        f" だから{own_line}"
    )
...
def _three_turn_grounded_surface(
    ...
    if turn_no == 2:
        ...
        if short_stance:
            return _short_b_turn2_repair(cfg, own_line, issue_primary, proposition_lock)
        return _sanitize_fighter_speech(
...
def _three_turn_validation_report(
    ...
    if turn_no == 1:
        ...
        if speaker == "B" and _needs_short_stance_boost(cfg, speaker):
            grounded_count = int(grounding.get("grounded_keyword_count") or 0)
            if grounded_count < 2:
                failures.append("short rebuttal lacks two concrete anchors")
            if ("副作用" in cleaned or "負担" in cleaned) and grounded_count < 2:
                failures.append("short rebuttal stays abstract")
            if any(token in cleaned for token in ["橋", "コンパス", "信号", "屋根", "灯台"]) and grounded_count < 2:
                failures.append("short rebuttal leans on metaphor")
    else:
...
def _should_force_short_b_rebuttal_repair(
    cfg: DebateConfig,
    speaker: str,
    turn_no: int,
    validation: dict[str, Any],
) -> bool:
    if speaker != "B" or turn_no != 2 or not _needs_short_stance_boost(cfg, speaker):
        return False
    failures = set(validation.get("three_turn_failures") or [])
    return bool(
        {
            "short rebuttal lacks two concrete anchors",
            "short rebuttal stays abstract",
            "short rebuttal leans on metaphor",
            "rebuttal lacks concrete support",
        }
        & failures
    )
