...
def _pure_turn_prompt(
    ...
    if turn_no == 2:
        return (
            ...
            "Break one core point.\n"
            "Expand your reasoning if needed.\n"
...
def _pure_turn_fallback(cfg: DebateConfig, speaker: str, turn_no: int, latest_opponent: str = "", transcript: str = "") -> str:
    if turn_no == 1:
    ...
    if turn_no == 2:
        short_clause = _turn2_opponent_short_clause(latest_opponent)
        topic = _clean_text(cfg.topic or "")
        position = cfg.side_a if speaker == "A" else cfg.side_b
        if "AI" in topic and "意識" in topic:
            point = "外から見える振る舞いと中で感じていることは同じではない"
        elif "AI" in topic and "得する人" in topic and "損する人" in topic:
            point = "仕事の移動と学び直しの遅れがまだ埋まっていない"
        elif "金" in topic and "銀" in topic:
            point = "実需の強さだけで長く持つ有利不利は決まらない"
        else:
            point = "決め手になる前提がまだ埋まっていない"
        if "AI" in topic and "意識" in topic:
            push = (
                "中の感覚が見えない限り、人間と同じ意識があるとは言えない。"
                if speaker == "A"
                else "同じように情報をまとめて自分を見返せるなら、意識に近い状態はあり得る。"
            )
        elif "AI" in topic and "得する人" in topic and "損する人" in topic:
            push = (
                "学び直しの遅れが埋まらない限り、損する側の方が増えやすい。"
                if speaker == "A"
                else "需要拡大と仕事の移動が進むなら、得する側の方が広がる。"
            )
        elif "金" in topic and "銀" in topic:
            push = (
                "実需の下支えが残るなら、銀の方が取りに行ける。"
                if speaker == "A"
                else "守りを重視するなら、金の方が有利だ。"
            )
        else:
            push = f"{_turn1_direct_claim(cfg.topic, position).rstrip('。')}。"
        if short_clause:
            weakness = f"その「{short_clause}」をそのまま結論にしているのが弱い。"
        else:
            weakness = "その押し方だと、まだ決め手にはならない。"
        analogy_line = _turn2_translation_analogy(f"{short_clause} {point}") if short_clause else ""
        return _sanitize_fighter_speech(
            f"{weakness} {point}。 {push}{(' ' + analogy_line) if analogy_line else ''}"
        )
    position = cfg.side_a if speaker == "A" else cfg.side_b
...
def _run_debate_pure(cfg: DebateConfig, session: _SessionRuntime) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        a_fallback = _pure_turn_fallback(cfg, "A", turn_no, a_latest_opponent, transcript)
        b_fallback = _pure_turn_fallback(cfg, "B", turn_no, b_latest_opponent, transcript)
        session.progress(
            stage="provider_call",
            turn=turn_no,
            active_speakers=["A", "B"],
