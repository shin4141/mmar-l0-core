...
def _opening_specific_example(cfg: DebateConfig, speaker: str, topic: str, concrete: str) -> str:
    ...
    if _opening_has_proper_noun(dense):
        return dense
    ...
    if lane == "science":
        if speaker == "A":
            return "SETIや系外惑星観測が積み上がっても、再現可能な人工信号や生命証拠はまだ出ていない。"
        return "可住域の惑星候補や有機分子の観測は増えており、存在可能性をゼロと断定する材料にもなっていない。"
    if lane == "product":
        if "saas" in topic.lower():
            if speaker == "A":
                return "実際に金利上昇局面ではCAC回収が重くなり、LTV/CACが割れたSaaS企業から失速が出た。"
            return "一方でVertical SaaSや高い切替コストが残る領域では、解約率が低く継続課金もまだ崩れていない。"
        return f"{anchor_a}をめぐる現場では、{anchor_b}の条件差がそのまま継続利用の差になっている。"
    if lane == "social":
        return f"{anchor_a}の制度運用では、{anchor_b}を現場で回せるかどうかが結論を分ける。"
    if "金より銀" in topic or ("銀" in topic and "金" in topic):
        if speaker == "A":
            return "銀は太陽光パネルや電子部品の需要が厚く、実需が残る局面では価格の支えになりやすい。"
        return "金は中央銀行の準備資産として買われやすく、景気が鈍る局面でも銀より需給が崩れにくい。"
    return concrete if concrete and len(_clean_text(concrete)) >= 20 else f"{anchor_a}と{anchor_b}の条件差が、そのまま結論の差になっている。"
...
def _short_stance_opening_surface(
    ...
    topic_terms = _opening_topic_terms(topic)
    anchor_a = topic_terms[0] if topic_terms else (lane_terms[0] if lane_terms else "この命題")
    anchor_b = topic_terms[1] if len(topic_terms) > 1 else (lane_terms[1] if len(lane_terms) > 1 else anchor_a)
    ...
    if lane == "science":
        ...
        support_reason = f"{primary_term}の決定打が無いままでは、{secondary_term}の可能性だけで存在を確定も否定もできないからだ。"
        attack_preempt = f"相手は{secondary_term}の広さだけで押してくるだろうが、{primary_term}が無いままでは一般命題にならない。"
    elif lane == "product":
        primary_term = lane_terms[0] if lane_terms else "市場の継続需要"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "切替コスト"
        support_reason = f"{primary_term}が崩れると{secondary_term}まで細り、成長の前提がそのまま弱るからだ。"
        attack_preempt = f"相手は追い風だけを強調するだろうが、{primary_term}と{secondary_term}の条件差まで見ないと結論は決まらない。"
    elif lane == "social":
        primary_term = lane_terms[0] if lane_terms else "制度運用"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "実装条件"
        support_reason = f"{anchor_a}が回らないと{anchor_b}も詰まり、制度全体が止まるからだ。"
        attack_preempt = f"相手は理念や単発の例外から崩そうとするだろうが、{anchor_a}と{anchor_b}の運用条件を越える反証にはなりにくい。"
    else:
        primary_term = lane_terms[0] if lane_terms else "現実条件"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "採用条件"
        support_reason = f"{primary_term}から{secondary_term}まで因果がつながるかどうかで、この立場の採用条件が決まるからだ。"
        attack_preempt = f"相手は抽象論だけで押すだろうが、{primary_term}と{secondary_term}が現実にどうつながるかを示さない限り結論は動かない。"
    if speaker == "A":
        return _sanitize_fighter_speech(
            f"{proposition}"
            f" 理由は、{support_reason}"
            f" {example_line}"
            f" だから{anchor_a}から{anchor_b}までの条件はまだ切れず、こちらの結論は崩れない。"
            f" {attack_preempt}"
        )
    return _sanitize_fighter_speech(
        f"{proposition}"
        f" 理由は、{support_reason}"
        f" {example_line}"
        f" だから{anchor_a}から{anchor_b}までの条件差が残る限り、相手の結論だけを先に確定することはできない。"
        f" {attack_preempt}"
    )
