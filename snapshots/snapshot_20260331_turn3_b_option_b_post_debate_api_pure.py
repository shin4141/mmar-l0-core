...
def run_debate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    debate, transcript, trace_entries = _run_debate_pure(cfg, session)
...
def _pure_turn_prompt(
    ...
    own_previous = _clean_text(_speaker_last_statement(speaker, turns))
    opponent_last = _clean_text(_opponent_last_statement(speaker, turns))
    if turn_no == 1:
        return _turn1_answer_prompt(cfg.topic, own_position)
    if turn_no == 2:
        return (
            ...
            "Schema: {\"speech\":\"...\",\"move\":\"rebuttal\",\"meta\":{\"phase\":\"rebuttal\",\"finish_intent\":\"push\",\"end_match\":\"no\"}}\n"
        )
    return (
        f"Full discussion:\n{transcript or '(none)'}\n\n"
...
def _pure_turn_fallback(cfg: DebateConfig, speaker: str, turn_no: int, latest_opponent: str = "", transcript: str = "", own_previous: str = "") -> str:
    if turn_no == 1:
        position = cfg.side_a if speaker == "A" else cfg.side_b
        return _sanitize_fighter_speech(
            f"{_turn1_direct_claim(cfg.topic, position)} "
            f"{_turn1_direct_support_reason(cfg.topic, position)} "
            f"{_turn1_direct_example(cfg.topic, position)}"
        )
    if turn_no == 2:
        core = _turn2_opponent_core_claim(latest_opponent)
        own_reason = _turn2_own_reinforcement(cfg, speaker, own_previous)
        weakness = _turn2_attack_line(cfg, speaker, latest_opponent, core)
        return _sanitize_fighter_speech(
            f"{weakness} {own_reason}"
        )
    position = cfg.side_a if speaker == "A" else cfg.side_b
    response = _turn3_latest_response(cfg, speaker, latest_opponent)
    gap = _turn3_gap(cfg, speaker)
    return _sanitize_fighter_speech(
...
def _compression_prompt_from_turns(turns: list[dict[str, Any]]) -> str:
    ...
    return (
        "以下の討論rawをUI表示用に圧縮せよ。\n"
        "目的は「説明を削り、勝負だけを残し、保存したくなる討論文にする」こと。\n"
        "各発話は120〜160字、最大200字。\n"
        "意味を足すな。立場を変えるな。新証拠を入れるな。\n"
        "削る基準: 説明、再証明、補足、前置き、transition語、論文口調。\n"
        "先頭は短い決定打から始めろ。\n"
        "決定打は『何が崩れたか』を即読できる一文にしろ。\n"
        "二文目は説明ではなく、立場を前に押す一文にしろ。\n"
        "Turn3は recap にせず、最後の押しが足りない理由を短く言ってから結論を固定しろ。\n"
        "短くしても熱は落とすな。整えるより、残る言い回しを優先しろ。\n"
        "ただし raw にない強み、証拠、比喩、論点を作るな。\n"
        "『したがって』『つまり』『要するに』などのつなぎ語でごまかすな。\n"
        "残す基準:\n"
        "- Turn1 = 結論が先、理由は一本だけ\n"
        "- Turn2 = 相手への一撃を先頭、自説の強化を二文目\n"
        "- Turn3 = 足りない理由を先頭、最後は結論固定\n"
        "避けるべき文:\n"
        "- 説明だけで終わる文\n"
        "- 立場が見えない文\n"
        "- どちらにも言えそうな一般論\n"
...
def _to_casual_pure(turn_no: int, text: str) -> str:
    ...
    if turn_no == 1 and len(sentences) >= 3:
        merged_second = f"{sentences[1]} {sentences[2]}".strip()
        sentences = [sentences[0], merged_second]
    elif turn_no == 3 and len(sentences) >= 3:
    ...
    for sentence in sentences:
        short = sentence
        short = short.replace("理由は、", "")
        short = short.replace("因果: ", "")
        short = short.replace("具体: ", "")
        short = short.replace("したがって、", "")
        clipped.append(short.strip())
    return _sanitize_fighter_speech(" ".join(clipped))


def _pure_turn_has_conclusion(cfg: DebateConfig, speaker: str, turn_no: int, text: str) -> bool:
    cleaned = _clean_text(text)
    if not cleaned:
        return False
    if turn_no == 1:
        expected = _turn1_direct_claim(cfg.topic, cfg.side_a if speaker == "A" else cfg.side_b).rstrip("。")
        return expected in cleaned or cleaned.endswith("。")
    if turn_no == 2:
        return "届かない" in cleaned or "決め手にならない" in cleaned or "押し切れない" in cleaned
    expected = _turn1_direct_claim(cfg.topic, cfg.side_a if speaker == "A" else cfg.side_b).rstrip("。")
    return "したがって" in cleaned and expected in cleaned


def _pure_turn_has_topic_contact(cfg: DebateConfig, speaker: str, turn_no: int, text: str, latest_opponent: str) -> bool:
    cleaned = _clean_text(text)
    if not cleaned:
        return False
    topic_terms = _opening_topic_terms(cfg.topic)
    if any(term in cleaned for term in topic_terms):
        return True
    if turn_no == 2:
        clause = _turn2_opponent_short_clause(latest_opponent)
        return bool(clause and clause in cleaned)
    own_claim = _turn1_direct_claim(cfg.topic, cfg.side_a if speaker == "A" else cfg.side_b).rstrip("。")
    return own_claim in cleaned


def _pure_turn_is_viable(cfg: DebateConfig, speaker: str, turn_no: int, text: str, latest_opponent: str) -> tuple[bool, list[str]]:
    cleaned = _clean_text(text)
    failures: list[str] = []
    if len(cleaned) < _pure_turn_min_chars(turn_no):
        failures.append("too short")
    if "。" not in cleaned and "！" not in cleaned and "?" not in cleaned and "？" not in cleaned:
        failures.append("not sentence-like")
    if not _pure_turn_has_topic_contact(cfg, speaker, turn_no, cleaned, latest_opponent):
        failures.append("no topic contact")
    if not _pure_turn_has_conclusion(cfg, speaker, turn_no, cleaned):
        failures.append("no conclusion")
    return not failures, failures


def _run_debate_pure(cfg: DebateConfig, session: _SessionRuntime) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    turns: list[dict[str, Any]] = []
    transcript = ""
    ...
    session.progress(stage="run_debate_start", extra={"turn_count": cfg.turn_count})
    for turn_no in range(1, cfg.turn_count + 1):
        stage_label = _stage_label(turn_no, cfg.turn_count)
        a_own_previous = _clean_text(_speaker_last_statement("A", turns))
        b_own_previous = _clean_text(_speaker_last_statement("B", turns))
        a_latest_opponent = _opponent_last_statement("A", turns)
        b_latest_opponent = _opponent_last_statement("B", turns)
        a_prompt = _pure_turn_prompt(cfg, "A", turns, transcript, turn_no)
        b_prompt = _pure_turn_prompt(cfg, "B", turns, transcript, turn_no)
        a_fallback = _pure_turn_fallback(cfg, "A", turn_no, a_latest_opponent, transcript, a_own_previous)
        b_fallback = _pure_turn_fallback(cfg, "B", turn_no, b_latest_opponent, transcript, b_own_previous)
        session.progress(
            stage="provider_call",
            turn=turn_no,
            active_speakers=["A", "B"],
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            a_data, _, _ = a_future.result()
            b_data, _, _ = b_future.result()
        a_text = _sanitize_fighter_speech(_clean_text(a_data.get("speech") or a_fallback))
        b_text = _sanitize_fighter_speech(_clean_text(b_data.get("speech") or b_fallback))
        if cfg.mode == "casual":
            a_text = _to_casual_pure(turn_no, a_text)
            b_text = _to_casual_pure(turn_no, b_text)
        if turn_no == 1:
            a_text = _normalize_turn1_assertion(a_text, cfg, "A")
            b_text = _normalize_turn1_assertion(b_text, cfg, "B")
        a_ok, a_failures = _pure_turn_is_viable(cfg, "A", turn_no, a_text, a_latest_opponent)
        b_ok, b_failures = _pure_turn_is_viable(cfg, "B", turn_no, b_text, b_latest_opponent)
        a_retry_triggered = False
        b_retry_triggered = False
        if not a_ok:
            a_retry_triggered = True
            retry = _speaker_turn_data("A", cfg.fighter_a_provider, a_prompt, cfg, a_fallback)
            a_text = _sanitize_fighter_speech(_clean_text(retry.get("speech") or a_fallback))
            if cfg.mode == "casual":
                a_text = _to_casual_pure(turn_no, a_text)
            if turn_no == 1:
                a_text = _normalize_turn1_assertion(a_text, cfg, "A")
            _apply_provider_status_delta(provider_statuses, cfg.fighter_a_provider, retry.get("_provider_status"))
            a_ok, a_failures = _pure_turn_is_viable(cfg, "A", turn_no, a_text, a_latest_opponent)
        if not b_ok:
            b_retry_triggered = True
            retry = _speaker_turn_data("B", cfg.fighter_b_provider, b_prompt, cfg, b_fallback)
            b_text = _sanitize_fighter_speech(_clean_text(retry.get("speech") or b_fallback))
            if cfg.mode == "casual":
                b_text = _to_casual_pure(turn_no, b_text)
            if turn_no == 1:
                b_text = _normalize_turn1_assertion(b_text, cfg, "B")
            _apply_provider_status_delta(provider_statuses, cfg.fighter_b_provider, retry.get("_provider_status"))
            b_ok, b_failures = _pure_turn_is_viable(cfg, "B", turn_no, b_text, b_latest_opponent)
        a_opening_ok = True if turn_no != 1 else a_ok
        b_opening_ok = True if turn_no != 1 else b_ok
        ...
        a_trace = {
            ...
            "sanitized_initial_text": a_text,
            "opening_validated": a_opening_ok,
            "opening_validation_failures": [] if a_opening_ok else a_failures,
            "retry_triggered": a_retry_triggered,
        ...
        b_trace = {
            ...
            "sanitized_initial_text": b_text,
            "opening_validated": b_opening_ok,
            "opening_validation_failures": [] if b_opening_ok else b_failures,
            "retry_triggered": b_retry_triggered,
            "repair_triggered": False,
            "adopted_stage": "initial",
            "final_text": b_text,
            "final_validation": {},
        }
        trace_entries.extend([a_trace, b_trace])
        turns.append(
            {
                "turn": turn_no,
                "stage_label": stage_label,
        ...
        transcript = _append_transcript(_append_transcript(transcript, turn_no, "A", a_text), turn_no, "B", b_text)
        round_debug.append(
            {
                "turn": turn_no,
                "a_prompt_preview": a_prompt[:200],
                "b_prompt_preview": b_prompt[:200],
                "a_visible_text": a_text,
                "b_visible_text": b_text,
            }
        )
    debate = {
        "topic": cfg.topic,
        ...
        "artifact_bundle_dir": "",
        "participants": {
            "a": {"side": cfg.side_a, "provider": cfg.fighter_a_provider},
            "b": {"side": cfg.side_b, "provider": cfg.fighter_b_provider},
        },
        "turns": turns,
        "round_debug": round_debug,
        "keyword_used_count": 0,
        "keyword_used_turns": [],
        "keyword_bonus_total": 0,
        "keyword_bonus_turns": [],
    }
...
def _finalize_fighter_output(
    ...
    if cfg.turn_count == 3 and turn_no >= 2 and (not stage1_pass or not stage2_pass):
        ...
        repair_candidate = _sanitize_fighter_speech(
            _three_turn_repair_speech(speaker, cfg, turns, turn_no, latest_opponent, finalized)
        )
        ...
        if chosen_label != "current":
            ...
            if turn_no >= 3 and chosen_label == "repair":
                plan = _mock_turn_plan(speaker, cfg, turns, turn_no, latest_opponent, finalized)
                debug_trace["repair_issue_retained"] = {
                    "target_issue": _clean_text(plan.get("target_issue") or ""),
                    "attacked_weakness": _clean_text(plan.get("attacked_weakness") or ""),
                    "new_issue": _clean_text(plan.get("new_issue") or ""),
                }
...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig, session: _SessionRuntime) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        b_latest_opponent = _opponent_last_statement("B", turns)
        if turn_no == 1:
            a_prompt = _turn1_answer_prompt(cfg.topic, cfg.side_a, _turn1_direct_claim(cfg.topic, cfg.side_a))
            b_prompt = _turn1_answer_prompt(cfg.topic, cfg.side_b, _turn1_direct_claim(cfg.topic, cfg.side_b))
        else:
        ...
        a_opening_sentence_fix_used = False
        if turn_no == 1:
            normalized_a_initial_text = _normalize_turn1_assertion(a_initial_text, cfg, "A")
            a_opening_sentence_fix_used = normalized_a_initial_text != a_initial_text
            a_initial_text = normalized_a_initial_text
        ...
        a_debug_trace = {
            ...
            "opening_sentence_fix_used": a_opening_sentence_fix_used,
            "retry_triggered": False,
            "repair_triggered": False,
            "adopted_stage": "initial",
        }
        if turn_no == 1:
            opening_ok, opening_failures = _is_valid_opening_speech(a_text, _opening_topic_terms(cfg.topic))
            ...
            if not opening_ok:
                ...
                retry_data = _speaker_turn_data(
                    ...
                    prompt=retry_prompt,
                    cfg=cfg,
                    fallback_speech=_turn1_direct_surface(cfg, "A"),
                )
                ...
                retry_raw = _clean_text(retry_data.get("speech") or _turn1_direct_surface(cfg, "A"))
                retry_text = _sanitize_fighter_speech(retry_raw)
                normalized_retry_text = _normalize_turn1_assertion(retry_text, cfg, "A")
                if normalized_retry_text != retry_text:
                    a_debug_trace["opening_sentence_fix_used"] = True
                retry_text = normalized_retry_text
                retry_ok, retry_failures = _is_valid_opening_speech(retry_text, _opening_topic_terms(cfg.topic))
                session.append_phase(
                ...
                if retry_ok:
                    ...
                    a_text = _turn1_direct_surface(cfg, "A")
                    a_validation = _three_turn_validation_report("A", cfg, turns, turn_no, a_text, a_latest_opponent) if cfg.turn_count == 3 else {}
        ...
        if (
            ...
            retry_data = _speaker_turn_data(
                ...
                prompt=retry_prompt,
                cfg=cfg,
                fallback_speech=_three_turn_repair_speech("A", cfg, turns, turn_no, a_latest_opponent, a_text),
            )
            _apply_provider_status_delta(provider_statuses, cfg.fighter_a_provider, retry_data.get("_provider_status"))
            session.append_phase(
            ...
            retry_post_started_at = time.time()
            retry_raw = _clean_text(retry_data.get("speech") or _three_turn_repair_speech("A", cfg, turns, turn_no, a_latest_opponent, a_text))
            retry_text = _sanitize_fighter_speech(retry_raw)
            ...
            if not a_validation.get("three_turn_contract_pass"):
                a_text = _three_turn_repair_speech("A", cfg, turns, turn_no, a_latest_opponent, a_text)
        ...
        if cfg.turn_count == 3:
            ...
            a_trace_entry = {
                ...
                "opening_sentence_fix_used": a_debug_trace.get("opening_sentence_fix_used"),
        ...
        b_opening_sentence_fix_used = False
        if turn_no == 1:
            normalized_b_initial_text = _normalize_turn1_assertion(b_initial_text, cfg, "B")
            b_opening_sentence_fix_used = normalized_b_initial_text != b_initial_text
            b_initial_text = normalized_b_initial_text
        ...
        b_debug_trace = {
            ...
            "opening_validated": turn_no != 1,
            "opening_validation_failures": [],
            "opening_sentence_fix_used": b_opening_sentence_fix_used,
            "retry_triggered": False,
            "repair_triggered": False,
            "adopted_stage": "initial",
        }
        if turn_no == 1:
            opening_ok, opening_failures = _is_valid_opening_speech(b_text, _opening_topic_terms(cfg.topic))
            ...
            if not opening_ok:
                ...
                retry_data = _speaker_turn_data(
                    ...
                    prompt=retry_prompt,
                    cfg=cfg,
                    fallback_speech=_turn1_direct_surface(cfg, "B"),
                )
                ...
                retry_post_started_at = time.time()
                retry_raw = _clean_text(retry_data.get("speech") or _turn1_direct_surface(cfg, "B"))
                retry_text = _sanitize_fighter_speech(retry_raw)
                normalized_retry_text = _normalize_turn1_assertion(retry_text, cfg, "B")
                if normalized_retry_text != retry_text:
                    b_debug_trace["opening_sentence_fix_used"] = True
                retry_text = normalized_retry_text
                retry_ok, retry_failures = _is_valid_opening_speech(retry_text, _opening_topic_terms(cfg.topic))
                session.append_phase(
                    "postprocess_opening_retry_evaluate",
                ...
                if retry_ok:
                    ...
                    b_text = _turn1_direct_surface(cfg, "B")
                    b_validation = _three_turn_validation_report("B", cfg, turns, turn_no, b_text, b_latest_opponent) if cfg.turn_count == 3 else {}
        ...
        if (
            ...
            b_debug_trace["forced_b_turn2_repair_skipped"] = True
        parity_failures = _three_turn_parity_failures(a_validation, b_validation) if cfg.turn_count == 3 else {"A": [], "B": []}
        if cfg.turn_count == 3 and turn_no != 1 and turn_no != 2 and parity_failures.get("A"):
            ...
            if not a_should_skip_parity_repair:
                repaired = _three_turn_repair_speech("A", cfg, turns, turn_no, a_latest_opponent, a_text)
                ...
                if repaired_validation.get("three_turn_contract_pass") and repaired_validation.get("density_score", 0) >= a_validation.get("density_score", 0):
                    ...
                    a_meta.update(a_debug_trace)
        if cfg.turn_count == 3 and turn_no != 1 and turn_no != 2 and parity_failures.get("B"):
        ...
        if cfg.turn_count == 3:
            ...
            b_trace_entry = {
                ...
                "opening_sentence_fix_used": b_debug_trace.get("opening_sentence_fix_used"),
...
def _speaker_prompt(
    ...
    opponent_last = _opponent_last_statement(speaker, turns) if turn_no != 1 else ""
    proposition_lock_block = _proposition_lock_prompt_block(cfg) if turn_no >= 3 else ""
    opening_contract_block = _opening_contract_prompt_block(cfg, speaker, turn_no) if turn_no >= 3 else ""
    three_turn_block = _three_turn_prompt_block(cfg, turn_no)
    turn1_stance_seed = _topic_grounded_short_claim(cfg, speaker) if turn_no == 1 else ""
    turn_mode_block = (
        "ANSWER MODE:\n"
        "- Turn 1 is not a debate. Turn 1 is a declaration.\n"
        "- Do not consider the opponent yet.\n"
        "- Do not balance or hedge.\n"
        "- Write the strongest possible answer for this position.\n"
        if turn_no == 1
        else "ATTACK MODE:\n"
        "- Turn 2 is direct combat.\n"
        ...
        "- Do not explain the analogy.\n"
        if turn_no == 2
        else "RECONSTRUCTION MODE:\n"
        "- Turn 3 and later may read the full debate history.\n"
        "- Respond directly to the opponent's latest counter.\n"
        "- Attack one specific weakness, then rebuild your own claim.\n"
        "- Push toward conclusion without drifting into a generic summary.\n"
        "- New issue: at most 1.\n"
    )
    context_block = (
        f"Topic: {cfg.topic}\n"
    ...
    goal_block = (
        "Goal: write the strongest possible answer for this position.\n"
        if turn_no == 1
        else "Goal: break the opponent's latest core.\n"
        if turn_no == 2
        else "Goal: rebuild your side and close the exchange.\n"
    )
    interaction_rules = (
        ""
        if turn_no == 1
        else "- Same-claim paraphrase is prohibited. Each turn must react to the opponent's immediate previous claim.\n"
        "- Use only one very short receive line for the opponent's last move. Do not summarize it at length.\n"
    )
    startup_rule = (
        "- Open with a forceful thesis, one causal chain, one topic-specific concrete, and one short preemption.\n"
        if turn_no == 1
        else "- Attack first. Do not rebuild your whole worldview yet.\n"
        if turn_no == 2
        else "- Respond to the latest counter first, then rebuild your claim and close.\n"
    )
    transcript_block = (
        ""
        if turn_no == 1
        else f"Opponent last statement:\n{opponent_last}\n"
        if turn_no == 2
        else f"Opponent last statement:\n{opponent_last}\nTranscript so far:\n{transcript or '(none yet)'}\n"
    )
    return (
        f"You are {role_name} in a structured debate prototype.\n"
        f"{_debate_axis_line(cfg.topic)}\n"
        "TOPIC LOCK:\n"
        ...
        "- Turn 1 is not a debate. Turn 1 is a declaration.\n"
        "- Turn 1 must sound like a confident opening claim, not a neutral summary.\n"
        "- Turn 1 must state the position strongly and directly.\n"
        "- The first sentence must be a direct declarative stance. It must not be phrased as a question. It must end as a plain assertion.\n"
        "- Turn 1 must include exactly these four things: one strong conclusion, one causal chain, one topic-specific concrete, and one short preemption.\n"
        ...
        "- Do not use analogies in Turn 1 or Turn 3.\n"
        "- In Turn 2, an analogy is optional and may appear at most once, as one short sentence, only if it makes the structural weakness clearer.\n"
        f"{_mode_prompt_rules(cfg.mode)}"
        f"{startup_rule}"
        f"{('- Turn 1 first sentence seed: ' + turn1_stance_seed + '\\n') if turn1_stance_seed else ''}"
...
def _debate_axis_line(topic: str) -> str:
    t = str(topic or "").strip().replace("？", "").replace("?", "")
    return f"Debate axis: {t}を争う。"


def _turn1_answer_prompt(topic: str, position: str, seed: str = "") -> str:
    clean_topic = _clean_text(topic or "")
    clean_position = _clean_text(position or "")
    return (
        f"Topic: {clean_topic}\n"
        f"Position: {clean_position}\n"
        "\n"
        "Answer the question directly in Japanese.\n"
        "\n"
        "- Start with a clear conclusion.\n"
        "- Add one reason.\n"
        "- Add one concrete example.\n"
        "- Do not use labels.\n"
        "- Do not explain debate structure.\n"
        "\n"
        "Write only the answer in natural Japanese.\n"
        "Return strict JSON only.\n"
        "Schema: {\"speech\":\"...\",\"move\":\"opening\",\"meta\":{\"phase\":\"opening\",\"finish_intent\":\"extend\",\"end_match\":\"no\"}}\n"
    )
...
def _three_turn_prompt_block(cfg: DebateConfig, turn_no: int) -> str:
    ...
    if turn_no == 1:
        base = (
            "- 3-turn mode.\n"
            "- Turn 1 = claim first: clear stance first, then one causal chain, one topic-specific concrete, and one short preemption.\n"
            "- Turn 1 is declaration mode: state your thesis strongly, not cautiously.\n"
            "- Do not begin with evaluation language, proof-criteria language, or opponent reference.\n"
            "- Turn 1 must show one causal chain, one concrete example, and one short preemption.\n"
            "- Forbidden in Turn 1: 一方で, ただし, 場合によっては, 〜もある, or any balancing language.\n"
            "- Forbidden in Turn 1: 〜と考えられる, 〜かもしれない, or any careful summary tone.\n"
        )
        if short_payload:
    ...
    if turn_no == 2:
        base = (
            ...
            "- At most one short analogy may appear after the weakness hit, and only if it sharpens the attack.\n"
            "- The analogy must come from a different everyday domain than the topic, such as food, tools, traffic, or games.\n"
            "- Do not explain the analogy.\n"
        )
        if short_payload:
    ...
    base = (
        "- Turn 3 = closing.\n"
        "- Follow this order: (1) name the opponent's latest counter in one sentence, (2) attack one specific weakness, (3) reinforce your own claim in one or two sentences, (4) close if needed.\n"
        "- You must respond directly to the opponent's latest counter.\n"
        "- You must attack one specific weakness.\n"
        "- You must reinforce your own claim.\n"
        "- Do not end with a generic summary.\n"
        "- Do not ignore the opponent.\n"
        "- New issue: at most 1.\n"
        "- Forbidden generic closings: 要するに, 結局, 総合すると, 全体として.\n"
    )
    if dignity_topic:
        base += (
...
def _fallback_speech(
    ...
    if cfg.turn_count == 3 and turn_no == 1:
        return _turn1_direct_surface(cfg, speaker)
    if cfg.turn_count == 3:
...
def _topic_lane(topic: str) -> str:
    ...
    low = cleaned.lower()
    if any(token in cleaned for token in ["宇宙", "神", "存在", "物理", "進化", "確率", "知性", "生命", "意識"]) or "alien" in low:
        return "science"
    if any(token in cleaned for token in ["金", "銀", "投資", "長期", "長期保有"]):
        return "asset"
    if any(token in cleaned for token in ["SaaS", "AI", "生成AI", "価格", "成長", "市場", "プロダクト", "事業", "動画サービス", "得", "損"]):
        return "product"
...
def _turn1_direct_claim(topic: str, position: str) -> str:
    cleaned_topic = _clean_text(topic or "").rstrip("。?？")
    cleaned_position = _clean_text(position or "")
    negative = any(token in cleaned_position for token in ["ない", "いない", "ではない", "終わりつつある", "損"])
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        return "AIに意識はない。" if negative else "AIに意識はある。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        return "損する人の方が多い。" if negative else "得する人の方が多い。"
    if "SaaS" in cleaned_topic and "終わり" in cleaned_topic:
        if any(token in cleaned_position for token in ["終わっていない", "終わりではない"]):
            return "SaaSは終わっていない。"
        return "SaaSは終わりつつある。"
    comparison_topic = any(token in cleaned_topic for token in ["より", "どちら", "向いている", "優れている"])
    if comparison_topic and "金" in cleaned_topic and "銀" in cleaned_topic:
        return "銀の方が有利だ。" if "銀" in cleaned_position else "金の方が有利だ。"
    if "存在する" in cleaned_topic:
        if any(token in cleaned_position for token in ["いない", "存在しない"]):
            return cleaned_topic.replace("存在する", "存在しない").rstrip("か") + "。"
        if any(token in cleaned_position for token in ["いる", "存在する"]):
            return cleaned_topic.rstrip("か") + "。"
    if cleaned_topic.endswith("か"):
        if 2 <= len(cleaned_position) <= 24 and not any(mark in cleaned_position for mark in "。！？?!"):
            return f"{cleaned_position}。"
        if any(token in cleaned_position for token in ["ない", "ではない", "いない", "不要"]):
            return f"{cleaned_topic[:-1]}とは言えない。"
        return f"{cleaned_topic[:-1]}。"
    if "は" in cleaned_topic and 1 < len(cleaned_position) <= 16:
        subject = cleaned_topic.split("は", 1)[0].strip()
        if subject:
            return f"{subject}は{cleaned_position}。"
    return f"{cleaned_position or cleaned_topic}。"


def _turn1_direct_support_reason(topic: str, position: str) -> str:
    cleaned_topic = _clean_text(topic or "")
    cleaned_position = _clean_text(position or "")
    negative = any(token in cleaned_position for token in ["ない", "いない", "ではない", "終わりつつある", "損"])
    if "宇宙人" in cleaned_topic and "銀河" in cleaned_topic:
        if negative:
            return "観測できる痕跡が空白のままなら、母数の大きさだけで実在までは言えない。"
        return "星の数に対して探せている範囲が狭いままなら、未検出だけで不在とは言えない。"
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        if negative:
            return "会話が自然でも、中の感覚が示せなければ意識があるとは言えない。"
        return "自分を振り返る応答が続くなら、意識を完全にゼロとは切れない。"
    if "AI" in cleaned_topic and "意思決定" in cleaned_topic:
        if negative:
            return "過去データの偏りをなぞるだけでは、例外や価値判断が要る場面で人間より良い判断にならない。"
        return "同じ条件をぶれずに処理できるほど、疲労や感情で揺れる人間より安定した判断を出せる。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        if negative:
            return "自動化が先に仕事を削り、学び直しが追いつかないと、得より先に損する側が広がる。"
        return "単純作業の負担が減り、新しい仕事へ移る人が増えるほど、得する側が広がる。"
    if "監視カメラ" in cleaned_topic and "犯罪抑止" in cleaned_topic:
        if negative:
            return "カメラが増えても現場対応が弱いままなら、犯罪は場所や手口を変えて残る。"
        return "見られる確率が上がるほど、その場でやる犯罪は割に合わなくなって減る。"
    if "SaaS" in cleaned_topic and "終わり" in cleaned_topic:
        if any(token in cleaned_position for token in ["終わっていない", "終わりではない"]):
            return "継続率が残る領域では値上げと利用定着が効き、解約率が低い企業から先に収益が粘る。"
        return "金利上昇でCACが悪化し、解約率が上がると、LTV/CACが崩れた企業から先に失速する。"
    if "金" in cleaned_topic and "銀" in cleaned_topic:
        if "銀" in cleaned_position:
            return "景気が弱くても実需の下支えが残る局面では、銀は下値を保ちやすい。"
        return "景気減速で産業需要が落ちると、銀は金より値動きが荒くなり、長く持つほど不利が出やすい。"
    topic_terms = _opening_topic_terms(cleaned_topic)
    first = topic_terms[0] if topic_terms else "条件"
    second = topic_terms[1] if len(topic_terms) > 1 else "結論"
    return f"{first}が崩れると{second}も崩れ、その差が結論を分ける。"


def _turn1_direct_example(topic: str, position: str) -> str:
    cleaned_topic = _clean_text(topic or "")
    cleaned_position = _clean_text(position or "")
    negative = any(token in cleaned_position for token in ["ない", "いない", "ではない", "終わりつつある", "損"])
    if "宇宙人" in cleaned_topic and "銀河" in cleaned_topic:
        if negative:
            return "今の観測では人工信号も接触痕跡も出ておらず、存在を押し上げる証拠はまだ見えていない。"
        return "天の川だけでも恒星は膨大で、その大半をまだ調べ切れていない。"
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        if negative:
            return "今のAIは自然に対話しても、痛みや欲求を自分で感じた証拠は示していない。"
        return "長い対話で自分の発言を振り返り続けるAIは増えており、境界は単純に切れなくなっている。"
    if "AI" in cleaned_topic and "意思決定" in cleaned_topic:
        if negative:
            return "採用や融資の審査では、AIが過去の偏りを学んで不公平な判断を繰り返した例がある。"
        return "需要予測や在庫管理では、AIが同じ条件を一気に処理して人より安定した判断を出している。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        if negative:
            return "自動レジでは会計の仕事は減っても、再就職や賃金の回復はすぐには追いついていない。"
        return "翻訳アプリや自動レジでは単純作業が減っても、保守や接客側の仕事は増えている。"
    if "監視カメラ" in cleaned_topic and "犯罪抑止" in cleaned_topic:
        if negative:
            return "繁華街ではカメラが増えても、死角や周辺道路へ犯罪が移る例がある。"
        return "駐車場やコンビニでは、カメラ設置後に万引きや車上荒らしが減る例がある。"
    if "SaaS" in cleaned_topic and "終わり" in cleaned_topic:
        if any(token in cleaned_position for token in ["終わっていない", "終わりではない"]):
            return "Vertical SaaSでは解約率と利用継続率がまだ粘り、値上げ後もシート課金が残っている。"
        return "CACと解約率を見ると、金利上昇後にLTV/CACが割れたSaaS企業から先に失速が出ている。"
    if "金" in cleaned_topic and "銀" in cleaned_topic:
        if "銀" in cleaned_position:
            return "銀は太陽光や電子部品の需要が残る局面で下値が支えられやすい。"
        return "金は景気後退局面でも中央銀行やETFの買いが入りやすい。"
    topic_terms = _opening_topic_terms(cleaned_topic)
    first = topic_terms[0] if topic_terms else "この論点"
    second = topic_terms[1] if len(topic_terms) > 1 else first
    return f"{first}をめぐる現場では、{second}の差が先に表れる。"


def _turn1_direct_preemption(topic: str, position: str) -> str:
    cleaned_topic = _clean_text(topic or "")
    cleaned_position = _clean_text(position or "")
    negative = any(token in cleaned_position for token in ["ない", "いない", "ではない", "終わりつつある", "損"])
    if "AI" in cleaned_topic and "意識" in cleaned_topic:
        if negative:
            return "先制: 「自然に話せるから意識がある」は、主観的経験の不在を落としている。"
        return "先制: 「まだ主観を証明できないからゼロだ」は、境界の曖昧さを落としている。"
    if "AI" in cleaned_topic and "得する人" in cleaned_topic and "損する人" in cleaned_topic:
        if negative:
            return "先制: 「生産性が上がれば皆得する」は、再配置の遅れを落としている。"
        return "先制: 「置き換えだけで損が広がる」は、再配置と新需要を落としている。"
    if "SaaS" in cleaned_topic and "終わり" in cleaned_topic:
        if any(token in cleaned_position for token in ["終わっていない", "終わりではない"]):
            return "先制: 「金利上昇で全部終わる」は、継続率が残る領域差を落としている。"
        return "先制: 「AI追い風でSaaSは全部伸びる」は、CACと解約率の悪化を落としている。"
    if "金" in cleaned_topic and "銀" in cleaned_topic:
        if "銀" in cleaned_position:
            return "先制: 「インフレなら金が自動的に強い」は、産業需要の残り方を落としている。"
        return "先制: 「インフレなら銀も強い」は、景気減速時の産業需要低下を落としている。"
    topic_terms = _opening_topic_terms(cleaned_topic)
    first = topic_terms[0] if topic_terms else "この論点"
    return f"先制: 「見た目だけで足りる」は、{first}の条件差を落としている。"


def _turn1_direct_surface(cfg: DebateConfig, speaker: str) -> str:
    position = cfg.side_a if speaker == "A" else cfg.side_b
    topic = _clean_text(cfg.topic or "")
    proposition = _turn1_direct_claim(topic, position)
    support_reason = _turn1_direct_support_reason(topic, position)
    example_line = _turn1_direct_example(topic, position)
    preemption = _turn1_direct_preemption(topic, position)
    return _sanitize_fighter_speech(
        f"{proposition} {support_reason} {example_line} {preemption}"
    )


def _turn2_translation_analogy(text: str) -> str:
    cleaned = _clean_text(text)
    if not cleaned:
        return ""
    mapping = [
        (("基準", "比較軸", "条件差"), "曲がった定規で測ってるのと同じだ。"),
        (("後付け", "ずれ", "すり替え"), "後からルール足してるのと同じだ。"),
        (("未応答", "答えていない", "閉じていない"), "質問に答えず信号を無視するようなものだ。"),
        (("一般化", "飛び", "伸びず"), "地図なしで迷わないと言ってるのと同じだ。"),
        (("コスト", "費用対効果", "負担"), "穴の空いたバケツに水を入れるのと同じだ。"),
    ]
    for needles, analogy in mapping:
        if any(needle in cleaned for needle in needles):
            return analogy
    return ""


def _turn2_opponent_core_claim(latest_opponent: str) -> str:
    cleaned = _clean_text(latest_opponent or "")
    if not cleaned:
        return ""
    cause_match = re.search(r"因果:\s*([^。]{10,80})", cleaned)
    if cause_match:
        return _clean_text(cause_match.group(1))[:48]
    short_clause = _turn2_opponent_short_clause(cleaned)
    if short_clause:
        return short_clause
    first = _first_sentence(cleaned)
    first = re.sub(r"^(具体|先制|理由|因果):\s*", "", first).strip()
    return _clean_text(first)[:40]


def _turn2_attack_line(cfg: DebateConfig, speaker: str, latest_opponent: str, core: str) -> str:
    topic = _clean_text(cfg.topic or "")
...
def _turn2_own_reinforcement(cfg: DebateConfig, speaker: str, own_previous: str = "") -> str:
    ...
    if "宇宙人" in topic and "銀河" in topic:
        if "存在しない" in own_position or "いない" in own_position:
            return "証拠が出ないままなら、存在主張だけが先に浮く。"
        return "探索が薄いままなら、不在断定の方が無理に走っている。"
    if "AI" in topic and "意思決定" in topic:
        if "できない" in own_position or "人間" in own_position:
            return "良い判断を分けるのは、精度より先に価値判断の筋だ。"
        return "同じ条件でぶれない判断なら、AIの方が一歩前に出る。"
    if "監視カメラ" in topic and "犯罪抑止" in topic:
        if "有効ではない" in own_position or "無効" in own_position:
            return "抑止と言うなら、その場で減ったと言える数字まで要る。"
        return "その場の犯罪を実際に減らせるなら、それだけで抑止の実益になる。"
    support = ""
    own_match = re.search(r"因果:\s*([^。]{10,80})", own_text)
    if own_match:
        support = _clean_text(own_match.group(1))
    position = cfg.side_a if speaker == "A" else cfg.side_b
    if not support:
        support = _turn1_direct_support_reason(cfg.topic, position)
    support = _clean_text(support.replace("因果:", "", 1))
    if support:
        return support if support.endswith(("。", "！", "？")) else f"{support}。"
    claim = _turn1_direct_claim(cfg.topic, position).rstrip("。")
    return f"だから、{claim}。"


def _turn2_opponent_short_clause(latest_opponent: str) -> str:
    cleaned = _clean_text(latest_opponent or "")
    if not cleaned:
        return ""
    first = _first_sentence(cleaned)
    first = re.sub(r"^相手の核は、?", "", first).strip()
    first = re.sub(r"という点だ。?$", "", first).strip()
    patterns = [
        r"(.{8,24}?が[^。]{0,10}?ない)",
        r"(.{8,24}?が[^。]{0,10}?ある)",
        r"(.{8,24}?が[^。]{0,10}?増える)",
...
def _turn3_latest_response(cfg: DebateConfig, speaker: str, latest_opponent: str) -> str:
    ...
    if "宇宙人" in topic and "銀河" in topic:
        ...
        return "最後まで残るのは、未検出をそのまま不在にはできないという一点だ"
    if "AI" in topic and "意思決定" in topic:
        if "できない" in own_position or "人間" in own_position:
            return "精度を積んでも、価値判断の穴は埋まらない"
        return "人間らしさを出しても、判断のぶれは残る"
    if "監視カメラ" in topic and "犯罪抑止" in topic:
        if "有効ではない" in own_position or "無効" in own_position:
...
def _turn3_gap(cfg: DebateConfig, speaker: str) -> str:
    ...
    if "宇宙人" in topic and "銀河" in topic:
        if "存在しない" in own_position or "いない" in own_position:
            return "存在を押し切る観測はまだ一つもない"
        return "不在を言い切るには、探索がまだ薄すぎる"
    if "AI" in topic and "意思決定" in topic:
        if "できない" in own_position or "人間" in own_position:
            return "良し悪しの基準は、まだ人間抜きで立たない"
        return "安定した判断力の差は、まだこちらに残る"
    if "監視カメラ" in topic and "犯罪抑止" in topic:
        if "有効ではない" in own_position or "無効" in own_position:
            return "減る場所より、残る犯罪の重さが消えない"
        return "減る犯罪がある事実は、まだ消せない"
    return "決め手までは埋まっていない"


def _turn2_latest_only_strike(latest_opponent: str, weakness_line: str = "", fatal_line: str = "") -> str:
    issue = _turn2_opponent_short_clause(latest_opponent) or "その一点"
    weakness = _clean_text(weakness_line or "")
    fatal = _clean_text(fatal_line or "")
    if not weakness:
        weakness = f"{issue}だけでは、そこから一般命題までは飛べない。"
    if not fatal:
        fatal = "そこで落ちている前提を埋めない限り、その反論は決め手にならない。"
    analogy_line = _turn2_translation_analogy(f"{weakness} {fatal}")
    return _sanitize_fighter_speech(
        f"{weakness}"
        f"{(' ' + analogy_line) if analogy_line else ''}"
        f" {fatal}"
    )
...
def _topic_grounded_short_claim(cfg: DebateConfig, speaker: str) -> str:
    ...
    comparison_topic = any(token in topic for token in ["より", "どちら", "向いている", "優れている"])
    ...
    if "存在する" in topic and own_line in {"いない", "存在しない"}:
        return f"{topic.replace('存在する', '存在しない').rstrip('か')}。"
    if "存在する" in topic and own_line in {"いる", "存在する"}:
        return f"{topic.rstrip('か')}。"
    if "終わりか" in topic:
        subject = topic.split("は", 1)[0].strip()
        if subject and own_line in {"終わりつつある", "終わっていない", "終わりではない"}:
            return f"{subject}は{own_line}。"
    if lane == "asset" and comparison_topic:
        if "銀" in own_line:
            if any(token in own_line for token in ["向いている", "有利", "伸びる", "取りに行ける"]):
                return "銀の方が有利だ。"
            return "銀の方が有利だ。"
        if "金" in own_line:
            if any(token in own_line for token in ["向いている", "有利", "残る", "守れる"]):
                return "金の方が有利だ。"
            return "金の方が有利だ。"
    if not topic:
        return f"{own_line}。"
...
def _opening_topic_terms(topic: str) -> list[str]:
    cleaned = _clean_text(topic)
    if "金" in cleaned and "銀" in cleaned and "長期保有" in cleaned:
        return ["金", "銀", "長期保有", "向いている"]
    return [term for term in _extract_focus_terms(topic) if len(term) >= 2][:4]


def _opening_has_proper_noun(text: str) -> bool:
    cleaned = _clean_text(text)
    return bool(re.search(r"[A-Z]{2,}|[A-Z][a-z]+|[0-9]{2,}", cleaned))


def _normalize_turn1_assertion(text: str, cfg: DebateConfig, speaker: str) -> str:
    cleaned = _clean_text(text)
    if not cleaned:
        return cleaned
    ...
    if not (
        ...
        or first.endswith("か？")
        or first.endswith("か?")
        or first.endswith("ではないか。")
        or first.endswith("ではないか？")
        or first.endswith("ではないか?")
    ):
        return cleaned
    position = cfg.side_a if speaker == "A" else cfg.side_b
    fixed_first = _turn1_direct_claim(cfg.topic, position)
    if fixed_first.endswith("か。") or fixed_first.endswith("か？") or fixed_first.endswith("か?"):
        fixed_first = fixed_first[:-2] + "。"
    if rest:
        return _sanitize_fighter_speech(f"{fixed_first} {rest}")
    return _sanitize_fighter_speech(fixed_first)


def _opening_first_sentence_is_question(text: str) -> bool:
    cleaned = _clean_text(text)
    if not cleaned:
        return False
...
def _opening_specific_example(cfg: DebateConfig, speaker: str, topic: str, concrete: str) -> str:
    proposition_lock = _build_proposition_lock(cfg)
    dense = _three_turn_concrete_support(speaker, proposition_lock, 1)
    ...
    if _opening_has_proper_noun(dense) and not any(pattern in dense for pattern in generic_example_patterns):
        return dense
    topic_terms = _opening_topic_terms(topic)
    anchor_a = topic_terms[0] if topic_terms else "この命題"
    anchor_b = topic_terms[1] if len(topic_terms) > 1 else anchor_a
    lane = _topic_lane(topic)
    if "金より銀" in topic or ("銀" in topic and "金" in topic):
        if speaker == "A":
            return "具体: 産業需要を見ると、銀は太陽光と電子部品の需要が残る局面で下値が支えられやすい。"
        return "具体: 安全資産需要を見ると、金は景気後退局面でも中央銀行とETFの買いが入りやすい。"
    if "得する人" in topic or "損する人" in topic or ("AI" in topic and ("得" in topic or "損" in topic)):
        if speaker == "A":
            return "具体: 自動レジでは会計の仕事は減っても、再就職や賃金の回復はすぐには追いついていない。"
        return "具体: 翻訳アプリや自動レジでは単純作業が減っても、保守や接客側の仕事は逆に増えている。"
    if lane == "science":
        if speaker == "A":
            return "具体: SETIや系外惑星観測が積み上がっても、再現可能な人工信号や生命証拠はまだ出ていない。"
        return "具体: 系外惑星や有機分子の観測データは増えていて、生命可能性をゼロと断定する根拠にもなっていない。"
    if lane == "product":
        if "saas" in topic.lower():
            if speaker == "A":
                return "具体: CACと解約率を見ると、金利上昇後にLTV/CACが割れたSaaS企業から先に失速が出ている。"
            return "具体: Vertical SaaSでは解約率と利用継続率がまだ粘り、値上げ後もシート課金が残っている。"
        return f"具体: {anchor_a}をめぐる現場では、{anchor_b}の条件差がそのまま継続率の差として表れている。"
    if lane == "social":
        return f"具体: {anchor_a}の制度運用では、{anchor_b}を現場で回せるかどうかが先に差として出る。"
    if concrete and len(_clean_text(concrete)) >= 20 and not any(pattern in concrete for pattern in generic_example_patterns):
        return f"具体: {_clean_text(concrete)}"
    return f"具体: {anchor_a}と{anchor_b}では、観測される条件差がそのまま結論の差として出る。"
...
def _short_stance_opening_surface(
    ...
    example_line = _opening_specific_example(cfg, speaker, topic, concrete)
    topic_lower = topic.lower()
    if "金" in topic and "銀" in topic:
        if speaker == "A":
            support_reason = "因果: 景気減速で産業需要が落ちても、銀は実需の下支えが残る局面では金ほど一方向に偏らない。"
            attack_preempt = "先制: 「インフレなら金が自動的に強い」は、産業需要の残り方を落としている。"
        else:
            support_reason = "因果: 景気減速で産業需要が落ちると、銀は金より値動きが荒くなり、長期保有では先に不利が出る。"
            attack_preempt = "先制: 「インフレなら銀も強い」は、景気減速時の産業需要低下を落としている。"
    elif lane == "science":
        if speaker == "A":
            support_reason = "因果: 観測データが増えても人工信号が出ないままなら、存在可能性は広がっても存在証明には進まない。"
            attack_preempt = "先制: 「候補が多いから存在する」は、再現可能な生命証拠の欠落を落としている。"
        else:
            support_reason = "因果: 系外惑星と有機分子の観測が増えるほど、生命可能性は広がり、ゼロ断定の方が先に苦しくなる。"
            attack_preempt = "先制: 「まだ見つからないからいない」は、観測範囲より結論を先に走らせている。"
    elif lane == "product":
        if "saas" in topic_lower:
            if speaker == "A":
                support_reason = "因果: 金利上昇でCACが悪化し、解約率が上がると、LTV/CACが崩れた企業から先に失速する。"
                attack_preempt = "先制: 「AI追い風でSaaSは全部伸びる」は、CACと解約率の悪化を落としている。"
            else:
                support_reason = "因果: 値上げ後も継続率が残り、解約率が低い領域では、Vertical SaaSから先に収益が粘る。"
                attack_preempt = "先制: 「金利上昇で全部終わる」は、継続率が残る領域差を落としている。"
        else:
            primary_term = lane_terms[0] if lane_terms else "継続率"
            secondary_term = lane_terms[1] if len(lane_terms) > 1 else "採算"
            support_reason = f"因果: {primary_term}が落ちると{secondary_term}も崩れ、先に失速する側がはっきり出る。"
            attack_preempt = f"先制: 「一時的な追い風で足りる」は、{primary_term}と{secondary_term}の差を落としている。"
    elif lane == "social":
        if "ai" in topic_lower and ("得" in topic or "損" in topic):
            if speaker == "A":
                support_reason = "因果: 自動化が進んでも再就職が遅れ、賃金圧迫が続くと、得より先に損を受ける側が広がる。"
                attack_preempt = "先制: 「生産性が上がれば皆得する」は、再就職と賃金圧迫の遅れを落としている。"
            else:
                support_reason = "因果: 自動化で単純作業の負担が減り、教育機会と再配置が進むほど、得を受ける人は広がる。"
                attack_preempt = "先制: 「置き換えだけで損が増える」は、再配置と教育機会の広がりを落としている。"
        else:
            primary_term = lane_terms[0] if lane_terms else "制度運用"
            secondary_term = lane_terms[1] if len(lane_terms) > 1 else "実装条件"
            support_reason = f"因果: {primary_term}が詰まると{secondary_term}も止まり、制度全体の効果が先に細る。"
            attack_preempt = f"先制: 「理念だけで足りる」は、{primary_term}と{secondary_term}の差を落としている。"
    else:
        primary_term = lane_terms[0] if lane_terms else "実質金利"
        secondary_term = lane_terms[1] if len(lane_terms) > 1 else "産業需要"
        support_reason = f"因果: {primary_term}が動くと{secondary_term}も変わり、その条件差が結論を分ける。"
        attack_preempt = f"先制: 「見た目の強さだけで足りる」は、{primary_term}と{secondary_term}の条件差を落としている。"
    if speaker == "A":
        return _sanitize_fighter_speech(
            f"{proposition}"
        ...
        if speaker == "A":
            if lane == "science":
                return _turn2_latest_only_strike(
                    latest_opponent,
                    f"{lane_term_a}や{lane_term_b}の観測は局所的な器用さや適応を示すだけで、火や金属加工や共同作業までは立証していない。",
                    "そこが飛んだままでは、一般命題まで伸ばす根拠にならない。",
                )
            if lane == "social":
                return _turn2_latest_only_strike(
                    latest_opponent,
                    f"{lane_term_a}や{lane_term_b}の現場を見ても、制度や運用を左右する具体条件はまだ詰め切れていない。",
                    "その穴が埋まらない限り、全体判断へは飛べない。",
                )
            if lane == "product":
                return _turn2_latest_only_strike(
                    latest_opponent,
                    f"{lane_term_a}や{lane_term_b}の具体例は一部の機能や運用を示すだけで、採用条件そのものを固定した証拠になっていない。",
                    "そこを飛ばしたままでは、市場全体の結論に届かない。",
                )
            if short_stance:
                return _turn2_latest_only_strike(
                    latest_opponent,
                    f"{short_hook}まで具体に置くと、一番重い現実条件をまだ崩せていない。",
                    "その一点が閉じない限り、その押し方では倒し切れない。",
                )
            return _turn2_latest_only_strike(
                latest_opponent,
                _three_turn_concrete_support("A", proposition_lock, 2),
                "その具体だけでは決定打にならず、命題までは崩れない。",
            )
...
def _speaker_last_statement(speaker: str, turns: list[dict[str, Any]]) -> str:
    if not turns:
        return "(none yet)"
    last_turn = turns[-1]
    if speaker == "A":
...
def _is_valid_opening_speech(text: str, topic_terms: list[str] | None = None) -> tuple[bool, list[str]]:
    cleaned = _clean_text(text)
    failures: list[str] = []
    if _opening_first_sentence_is_question(cleaned):
        failures.append("opening first sentence is question")
    first_sentence = re.split(r"(?<=[。.!?！？])\s*", cleaned, maxsplit=1)[0].strip()
    comparison_topic = any(term in (topic_terms or []) for term in ["金", "銀", "長期保有", "向いている"])
    forbidden_first_sentence_tokens = ["長期", "短期", "場合", "なら", "では"]
    if comparison_topic and any(token in first_sentence for token in forbidden_first_sentence_tokens):
        failures.append("opening first sentence carries comparison condition")
    has_conclusion = bool(re.match(r"^.+?[。.!?！？]", cleaned))
    has_causal = any(token in cleaned for token in ["理由", "なぜなら", "だから", "そのため", "結果として", "ので", "因果:"])
    has_concrete = "具体:" in cleaned or ("例えば" in cleaned and _opening_has_proper_noun(cleaned))
    has_preempt = "先制:" in cleaned
    structural_opening = has_conclusion and has_causal and has_concrete and has_preempt
    if len(cleaned) < 120 and not structural_opening:
        failures.append("opening too short")
    if not has_causal:
        failures.append("opening lacks causal explanation")
    terms = [term for term in (topic_terms or []) if term]
    expanded_terms: list[str] = []
    for term in terms[:4]:
        if term not in expanded_terms:
            expanded_terms.append(term)
        for subterm in re.split(r"(より|の方が|長期保有|向いている)", term):
            subterm = _clean_text(subterm)
            if len(subterm) >= 1 and subterm not in {"より", "の方が"} and subterm not in expanded_terms:
                expanded_terms.append(subterm)
    matched_terms = [term for term in expanded_terms if term and term in cleaned]
    comparison_topic = "金" in terms and "銀" in terms and "長期保有" in terms
    if comparison_topic:
        comparison_hits = sum(
            1 for term in ["金", "銀", "長期保有", "向いている"] if term in cleaned
        )
        required_topic_hits = 2
        if comparison_hits >= required_topic_hits:
            matched_terms.extend(["__comparison_ok__"] * required_topic_hits)
    else:
        required_topic_hits = 2 if len(expanded_terms) >= 2 else 1
    if terms and len(matched_terms) < required_topic_hits:
        failures.append("opening lacks topic anchor")
    if "例えば" in cleaned and not _opening_has_proper_noun(cleaned):
        failures.append("opening example lacks proper noun")
    if cleaned.startswith(("得する。", "損する。", "理由は", "まだ切れない", "この命題")):
        failures.append("opening starts with weak generic scaffold")
    if any(pattern in cleaned for pattern in GENERIC_OPENING_PATTERNS):
        failures.append("opening contains generic template")
    return not failures, failures
...
def _three_turn_repair_speech(
    ...
    topic = _clean_text(cfg.topic or "")
    if cfg.turn_count == 3 and turn_no >= 3:
        own_line = _clean_text(cfg.side_a if speaker == "A" else cfg.side_b) or "こちらの立場は残る。"
        opponent_line = _clean_text(latest_opponent or _opponent_last_statement(speaker, turns))
        current = _sanitize_fighter_speech(current_text or "")
        plan = _mock_turn_plan(speaker, cfg, turns, turn_no, latest_opponent, current)
        target_issue = _clean_text(plan.get("target_issue") or "") or "その一点"
        attacked_weakness = _clean_text(plan.get("attacked_weakness") or "") or "決め手が閉じていない"
        new_issue = _clean_text(plan.get("new_issue") or "")
        weakness_effect = _weakness_effect(attacked_weakness)
        generic_closings = ("要するに", "結局", "総合すると", "全体として")

        response_sentence = ""
        if current:
            for sentence in re.split(r"(?<=[。.!?！？])\s*", current):
                clean_sentence = _clean_text(sentence)
                if not clean_sentence or any(token in clean_sentence for token in generic_closings):
                    continue
                if "相手" in clean_sentence or "あなた" in clean_sentence:
                    response_sentence = clean_sentence
                    break
        if response_sentence.startswith("相手の核は、相手の核は、"):
            response_sentence = response_sentence.replace("相手の核は、相手の核は、", "相手の核は、", 1)
        if "という点だという点だ" in response_sentence:
            response_sentence = response_sentence.replace("という点だという点だ", "という点だ")
        if not response_sentence:
            opponent_core = _first_sentence(opponent_line).rstrip("。")
            opponent_core = re.sub(r"^相手の核は、?", "", opponent_core)
            opponent_core = re.sub(r"という点だ$", "", opponent_core)
            if opponent_core:
                response_sentence = f"相手の核は、{opponent_core}という点だ。"
            else:
                response_sentence = "相手の最新反論は、その一点で押し切れるという点だ。"

        weakness_sentence = (
            f"だが弱いのは、{target_issue}を決め手にしながら、{attacked_weakness}という点で、"
            f"{weakness_effect}。"
        )
        if new_issue:
            push_sentence = (
                f"さらに{new_issue}まで見ると、相手の押し返しはまだ閉じておらず、"
                f"{own_line}というこちらの主張は崩れない。"
            )
        else:
            push_sentence = f"その穴が残る以上、{own_line}というこちらの主張は崩れない。"
        close_sentence = f"だから最後に残るのは、{own_line}というこちらの結論だ。"

        repaired = _sanitize_fighter_speech(
            f"{response_sentence} {weakness_sentence} {push_sentence} {close_sentence}"
        )
        for pattern in generic_closings:
            repaired = repaired.replace(pattern, "")
        return _sanitize_fighter_speech(repaired)
    if (
        speaker == "B"
        and turn_no == 2
        and _needs_short_stance_boost(cfg, "B")
        ...
        for term in opponent_terms:
            ...
            if len(concrete_terms) >= 4:
                break
        concrete_a = concrete_terms[0] if concrete_terms else "その前提"
        concrete_b = concrete_terms[1] if len(concrete_terms) > 1 else "その条件"
        if "金" in topic and "銀" in topic:
            return _turn2_latest_only_strike(
                latest_opponent,
                "銀は太陽光需要で下支えされるだけでは、長期保有の優位までは固定できない。",
                "流動性と担保実務を落としたままでは、銀優位の主張は決め手にならない。",
            )
        return _turn2_latest_only_strike(
            latest_opponent,
            f"{_turn2_opponent_short_clause(latest_opponent) or concrete_a}だけでは、そこから一般命題までは飛べない。",
            f"{concrete_b}で落ちている前提を埋めない限り、その反論は決め手にならない。",
        )
