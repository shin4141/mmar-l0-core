...
def _paragraph_ok(turn_name: str, text: str, thesis: str, anti_thesis: str) -> bool:
    ...
    if not normalized:
        return False
    if _paragraph_forbidden_text(normalized):
        return False
...
def _turn_prompt(
    *,
    ...
    if turn_name == "turn1":
        spec = (
            "Write one short Japanese debate opening as a single paragraph of 2 to 3 sentences.\n"
            "- Sentence 1 must hit immediately: stake the side and the wound in one sharp line.\n"
            "- Sentence 2 must name one strongest reason only, with concrete force.\n"
            "- Sentence 3 must give one topic-specific example or observation, not a generic trend line.\n"
            "- Prefer pressure and specificity over balance or careful explanation.\n"
            "- Do not write formulaic labels such as 「痛点は」「現に」.\n"
        )
    elif turn_name == "turn2":
        spec = (
            "Write one short Japanese rebuttal as a single paragraph of 2 to 3 sentences.\n"
            "- Attack one fatal flaw in the opponent's latest turn. Make it specific to what they just said, not a restatement of your own case.\n"
            "- Use one concrete metaphor that fits this topic; avoid generic metaphors like collapsing dams, storms, or abstract explosions.\n"
            "- End by driving your own thesis forward with pressure, not by merely restating it flatly.\n"
        ...
        spec = (
            ...
            "- Show one thing the opponent still has not proved.\n"
            "- Keep pressure on the one reason your side still stands.\n"
...
def run_debate_v4(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        a1, a_status = _attempt_paragraph_generation(
            turn_name="turn1",
            prompt=_turn_prompt(
                topic=topic,
        ...
        b1, b_status = _attempt_paragraph_generation(
            turn_name="turn1",
            prompt=_turn_prompt(
                topic=topic,
        ...
        a2, a_status = _attempt_paragraph_generation(
            turn_name="turn2",
            prompt=_turn_prompt(
                topic=topic,
                role=card["side_a_role"],
        ...
        provider_statuses["openai_a"] = a_status
        b2, b_status = _attempt_paragraph_generation(
            turn_name="turn2",
            prompt=_turn_prompt(
                topic=topic,
                role=card["side_b_role"],
        ...
        a3, a_status = _attempt_paragraph_generation(
            turn_name="turn3",
            prompt=_turn_prompt(
                topic=topic,
                role=card["side_a_role"],
        ...
        provider_statuses["openai_a"] = a_status
        b3, b_status = _attempt_paragraph_generation(
            turn_name="turn3",
            prompt=_turn_prompt(
                topic=topic,
                role=card["side_b_role"],
