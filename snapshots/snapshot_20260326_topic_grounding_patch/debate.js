...
function setReadingMode(active) {
  ...
  if (readerControlsEl) {
    ...
    readerControlsEl.style.display = "none";
  }
}
...
function clearCurrentResultView() {
  ...
  outputMetaEl.hidden = true;
  outputMetaEl.style.display = "none";
...
function collectPayload() {
  const topic = document.querySelector("#topic").value.trim();
  const generatedPositions = generatedPositionsFromTopic(topic);
  const sideA = document.querySelector("#side-a").value.trim() || generatedPositions.a;
  const sideB = document.querySelector("#side-b").value.trim() || generatedPositions.b;
