...
def run_debate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    debate, provider_statuses, trace_entries = _run_debate_with_provider_fallbacks(cfg)
...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig) -> tuple[dict[str, Any], dict[str, dict[str, str]], list[dict[str, Any]]]:
    ...
    return debate, provider_statuses, trace_entries
...
def _three_turn_rebuttal_surface(
    ...
    if speaker == "A":
        return _sanitize_fighter_speech(
            f"相手の押し込みは見えやすい一点に寄りすぎている。{_three_turn_concrete_support('A', proposition_lock, 2)} だから{own_line}"
        )
    return _sanitize_fighter_speech(
        f"相手は結論を守りたいが、必要条件を閉じ切れていない。{_three_turn_concrete_support('B', proposition_lock, 2)} だから{own_line}"
    )
...
def _three_turn_closing_surface(
    ...
    if speaker == "A":
        return _sanitize_fighter_speech(
            f"ここまで来ると、相手は決め手にした一点から先を支え切れていない。条件まで通して見れば、最後に残るのは{own_line}"
        )
    return _sanitize_fighter_speech(
        f"結局、相手は立場を最後まで立たせる条件を閉じ切れていない。そこが埋まらない限り、残る結論は{own_line}"
    )
