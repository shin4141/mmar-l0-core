...
const PUBLIC_INTERACTIVE_DEFAULT_POSITIONS = {
  a: "この命題に賛成。採用する理由と実用上の利点があると主張する。",
  b: "この命題に反対。前提の弱さや副作用があるため慎重であるべきだと主張する。",
};
...
function applyPublicInteractiveDefaults() {
  ...
  if (sideAInput && !sideAInput.value.trim()) sideAInput.value = PUBLIC_INTERACTIVE_DEFAULT_POSITIONS.a;
  if (sideBInput && !sideBInput.value.trim()) sideBInput.value = PUBLIC_INTERACTIVE_DEFAULT_POSITIONS.b;
}
...
async function runPublicFixedDemo() {
  ...
  try {
    publicFixedDemoLog("fixture_loader_entered");
    const data = await loadPublicFixedDemoResult();
    renderResult(data);
    publicFixedDemoLog("fixture_loader_completed");
    setStatus("ok", "Debate ready");
    setHint("");
  } catch (error) {
    publicFixedDemoLog("fixture_loader_failed", String(error?.message || error));
    currentResult = null;
    currentRecordId = null;
    currentLoadedRecord = null;
    setRevealState(true);
...
function clearPublicSummary() {
  if (!publicSummaryEl || !publicSummaryWinnerEl || !publicSummaryReasonEl) return;
  publicSummaryEl.hidden = true;
  publicSummaryWinnerEl.textContent = "-";
  publicSummaryReasonEl.textContent = "-";
}

function renderPublicSummary(summary) {
  if (!publicSummaryEl || !publicSummaryWinnerEl || !publicSummaryReasonEl) return;
  const winner = normalizeWinner(summary || {});
  publicSummaryWinnerEl.textContent = winner.side || "Draw";
  publicSummaryReasonEl.textContent = summary?.reason_one_liner || winner.reason || "未生成";
...
function renderResult(result) {
  ...
  setRevealState(true);
  setReadingMode(true);
  renderPublicSummary(currentResult?.debate?.summary || {});
  renderRuntimeFingerprint();
  refreshOutput();
  renderDebugPipeline();
  syncSaveButton();
