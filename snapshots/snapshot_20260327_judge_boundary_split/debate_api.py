...
def run_debate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    debate, transcript, provider_statuses, trace_entries, phase_entries = _run_debate_with_provider_fallbacks(cfg)
    ...
    debate["summary"] = _session_inline_judge_summary(cfg, debate.get("turns") or [], transcript, provider_statuses)
    phase_entries.append(_phase_entry("judge_summary", judge_started_at))
    phase_entries.append(_phase_entry("run_debate_total", phase_entries[0]["at"], completed_turns=len(debate.get("turns") or [])))
    _write_progress_checkpoint(
        cfg,
        stage="completed",
        phase_entries=phase_entries,
...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig) -> tuple[dict[str, Any], str, dict[str, dict[str, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    return debate, transcript, provider_statuses, trace_entries, phase_entries
...
def _session_inline_judge_summary(
    ...
    if cfg.disable_live_judge:
        ...
        return fallback
    return _judge_summary_data(cfg, turns, transcript, provider_statuses)
