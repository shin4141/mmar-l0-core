...
def _pure_turn_prompt(
    ...
    if turn_no == 2:
        return (
            ...
            "Attack one point directly.\n"
            "Then push your position back in one short sentence.\n"
            "At most one short analogy is allowed if it sharpens the hit.\n"
...
def _pure_turn_fallback(cfg: DebateConfig, speaker: str, turn_no: int, latest_opponent: str = "", transcript: str = "") -> str:
    ...
    if turn_no == 2:
        short_clause = _turn2_opponent_short_clause(latest_opponent) or "その一点"
        topic = _clean_text(cfg.topic or "")
        position = cfg.side_a if speaker == "A" else cfg.side_b
        if "AI" in topic and "意識" in topic:
            point = "観察できる振る舞いと主観的経験は同じではない"
        elif "AI" in topic and "得する人" in topic and "損する人" in topic:
            point = "再配置と再訓練の遅れがまだ埋まっていない"
        elif "金" in topic and "銀" in topic:
            point = "実需だけで長期保有の優位は決まらない"
        else:
            point = "決め手になる前提がまだ埋まっていない"
        if "AI" in topic and "意識" in topic:
            push = (
                "主観的経験が示されない限り、人間と同じ意識があるとは言えない。"
                if speaker == "A"
                else "機能的に同等の情報統合があれば、意識に相当する状態は成立し得る。"
            )
        elif "AI" in topic and "得する人" in topic and "損する人" in topic:
            push = (
                "再訓練の遅れが埋まらない限り、損する側の方が増えやすい。"
                if speaker == "A"
                else "需要拡大と再配置が進むなら、得する側の方が広がる。"
            )
        elif "金" in topic and "銀" in topic:
            push = (
        ...
        weakness = f"その「{short_clause}」だけでは、結論までは届かない。"
        fatal = f"理由は、{point}がまだ埋まっていないからだ。"
        analogy_line = _turn2_translation_analogy(f"{short_clause} {point}")
        return _sanitize_fighter_speech(
            f"{weakness} {push}{(' ' + analogy_line) if analogy_line else ''}"
        )
    position = cfg.side_a if speaker == "A" else cfg.side_b
...
def _to_casual_pure(turn_no: int, text: str) -> str:
    ...
    if turn_no == 3 and len(sentences) > 2:
