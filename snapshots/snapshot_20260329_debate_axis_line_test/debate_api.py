...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig, session: _SessionRuntime) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        if turn_no == 1:
            a_prompt = _turn1_answer_prompt(cfg.topic, cfg.side_a, _topic_grounded_short_claim(cfg, "A"))
            b_prompt = _turn1_answer_prompt(cfg.topic, cfg.side_b, _topic_grounded_short_claim(cfg, "B"))
        else:
...
def _normalize_turn1_assertion(text: str, cfg: DebateConfig, speaker: str) -> str:
    ...
    if not (
        ...
        return cleaned
    fixed_first = _topic_grounded_short_claim(cfg, speaker)
