...
def run_live_judge(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    judge_phase = [_phase_entry("judge_summary", judge_started_at)]
    judge_info = provider_statuses.get("judge", {})
    ...
    if judge_mode != "live":
        ...
        return {
            ...
            "mode": "judge-failed",
            "provider_statuses": provider_statuses,
            "artifact_phase_entries": judge_phase,
        }
    return {
        "ok": True,
        "mode": "live",
    ...
    *,
    ...
    try:
        ...
        payload = {
            ...
            "request_model": OPENAI_MODEL if provider == "openai" else (ANTHROPIC_MODEL if provider == "anthropic" else _default_gemini_model_name()),
...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig) -> tuple[dict[str, Any], dict[str, dict[str, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        phase_entries.append({"name": "turn_phase_start", "at": turn_phase_started_at, "turn": turn_no})
        _write_progress_checkpoint(
            cfg,
            stage="provider_call",
            phase_entries=phase_entries,
        ...
        phase_entries.append(_phase_entry("turn_phase_total", turn_phase_started_at, turn=turn_no))
        ...
        _append_phase_entry(phase_entries, "postprocess_transcript_append", transcript_append_started_at, turn=turn_no)
        ...
        _append_phase_entry(phase_entries, "postprocess_turn_record_append", turns_append_started_at, turn=turn_no)
        _write_progress_checkpoint(
            cfg,
...
def _judge_summary_data(
    ...
    judge_model_name = _resolve_gemini_model(cfg.gemini_key) if cfg.gemini_key else _default_gemini_model_name()
...
def _speaker_prompt(
    ...
    return (
        ...
        "- meta is hidden scaffold only. Do not surface planning labels or debate-notes wording inside speech.\n"
        ...
        "- Turn 1 is opening: assert your thesis first and ground it in topic-specific concrete points.\n"
        "- In Turn 1, do not open with evaluation language, battlefield-definition language, or judge-like criteria language.\n"
        "- In Turn 1, do not begin by talking about the opponent or about what counts as proof.\n"
        f"{opening_contract_block}"
        "- Turn 2 is rebuttal: directly counter the opponent opening and set the first pressure point.\n"
        "- Turn 3 is deepening: receive one concrete point from the opponent, answer it directly, reinforce your stance, and close on the issue that still remains.\n"
        ...
        "- Do not use binary answer labels or verdict labels inside the speech body. Just make the argument itself.\n"
        ...
        "- Keep A and B equally forceful. Do not let one side shrink into a thin template line while the other side carries all the specifics.\n"
...
def _three_turn_prompt_block(cfg: DebateConfig, turn_no: int) -> str:
    ...
    if turn_no == 1:
        base = (
            ...
            "- Turn 1 must begin in claim mode: clear stance first, then at least two topic-specific concrete supports.\n"
            "- Do not begin with battlefield setup, evaluation criteria, acceptance-condition language, or a slogan.\n"
            "- Keep Turn 1 free of opponent reference in the opening line.\n"
        )
        if dignity_topic:
            base += (
...
def _opening_contract_prompt_block(cfg: DebateConfig, speaker: str, turn_no: int) -> str:
    ...
    return (
        "- For A Turn 1, keep the opening contract hidden in meta instead of surfacing it as visible setup prose.\n"
        "- Start by asserting the claim itself in natural Japanese.\n"
        "- After the claim, name two topic-specific concrete supports.\n"
        "- Do not surface labels such as 見たい筋, 焦点, 比較軸, 採用条件, or anti-reframe guard in the speech body.\n"
        ...
        f"- Hidden anti-reframe guard for meta only: {contract['anti_reframe_guard']}\n"
        f"- Hidden exception policy for meta only: {contract['exception_policy']}\n"
        f"- Hidden burden target for meta only: {contract['burden_target']}\n"
...
def _three_turn_grounded_surface(
    ...
    if turn_no == 2:
        if speaker == "A":
            if short_stance:
                return _sanitize_fighter_speech(
                    f"相手は「{opponent_focus}」で押してくるが、まだ届いていない。"
                    f" {short_hook}まで含めて見ると、相手は一番重い具体物をまだ崩せていない。"
                    f" {short_metaphor}。"
                    f" だから{own_line}"
                )
            return _sanitize_fighter_speech(
                f"相手は「{opponent_focus}」でこちらを崩したいが、まだ届いていない。{_three_turn_concrete_support('A', proposition_lock, 2)} だから{own_line}"
            )
