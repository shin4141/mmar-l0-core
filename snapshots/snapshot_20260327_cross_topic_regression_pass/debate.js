...
async function runDebate(event) {
  ...
  setRunMeta("Running debate...", true);
  ...
  try {
    ...
    if (VIEWER_MODE) {
      ...
      setHint(String(error?.message || "The debate could not be generated. Please check the selected models and API keys, then try again."));
    }
  } finally {
    runButton.disabled = false;
    checkApiHealth();
