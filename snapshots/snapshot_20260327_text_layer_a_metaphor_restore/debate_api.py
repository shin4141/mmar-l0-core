        ...
        if speaker == "A":
            if "原発" in topic or "電力" in topic or "再エネ" in topic:
                return _sanitize_fighter_speech(
                    "見たい筋は「実行条件」だ。"
                    " 実行条件で私は原発維持を支持するが崩れずに残るなら、この立場は成立する。"
                    " 原発は短期の安定供給と低炭素の基幹電源としてまだ価値があり、LNG輸入増と電力価格高騰だけで穴埋めする方が家計と産業の負担は重い。"
                    " 問題は危険の有無ではなく、安全規制、廃炉積立、賠償枠、最終処分の計画を現実に閉じられるかだ。"
                    " 灯りを消す前に、代わりの発電所と金庫を先に用意できるなら維持の理由は残る。"
                    f" だから{own_line}"
                )
            return _sanitize_fighter_speech(
                f"見たい筋は「{issue_primary}」です。{contract.get('acceptance_condition') or 'この条件が満たされるなら立場は残る。'} {concrete} だから{own_line}"
            )
        return _sanitize_fighter_speech(
            f"私は{own_line}。{concrete} 相手がここを越えられない限り、この命題は立たない。"
        )
    if turn_no == 2:
        if speaker == "A":
            return _sanitize_fighter_speech(
        ...
        return _sanitize_fighter_speech(
            f"相手は「{opponent_focus}」の負担をまだ処理していない。{_three_turn_concrete_support('B', proposition_lock, 2)} だから{own_line}"
        )
    if speaker == "A":
        if "原発" in topic or "電力" in topic or "再エネ" in topic:
            return _sanitize_fighter_speech(
