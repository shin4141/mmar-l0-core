...
def run_debate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    debate, provider_statuses, trace_entries, phase_entries = _run_debate_with_provider_fallbacks(cfg)
    ...
    return {
        ...
        "artifact_phase_entries": phase_entries,
    }
...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig) -> tuple[dict[str, Any], dict[str, dict[str, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    phase_entries.append({"name": "run_debate_start", "at": run_started_at, "turn_count": cfg.turn_count})
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        a_provider_started_at = time.time()
        ...
        phase_entries.append(_phase_entry("provider_call", a_provider_started_at, turn=turn_no, speaker="A", provider=cfg.fighter_a_provider, provider_mode=a_data.get("_provider_mode") or ""))
        a_postprocess_started_at = time.time()
        a_initial_raw = _clean_text(a_data.get("speech") or _fallback_speech("A", cfg, turns, turn_no, a_latest_opponent))
        ...
        if cfg.turn_count == 3 and not a_validation.get("three_turn_contract_pass"):
        ...
        phase_entries.append(_phase_entry("postprocess", a_postprocess_started_at, turn=turn_no, speaker="A"))

        b_prompt = _speaker_prompt("B", cfg.fighter_b_provider, cfg, turns, prior_transcript, turn_no, stage_label)
        b_provider_started_at = time.time()
        ...
        phase_entries.append(_phase_entry("provider_call", b_provider_started_at, turn=turn_no, speaker="B", provider=cfg.fighter_b_provider, provider_mode=b_data.get("_provider_mode") or ""))
        b_postprocess_started_at = time.time()
        b_initial_raw = _clean_text(b_data.get("speech") or _fallback_speech("B", cfg, turns, turn_no, b_latest_opponent))
        ...
        if cfg.turn_count == 3 and not b_validation.get("three_turn_contract_pass"):
            ...
            retry_data = _speaker_turn_data(
                ...
                prompt=retry_prompt,
                cfg=cfg,
                fallback_speech=_three_turn_repair_speech("B", cfg, turns, turn_no, b_latest_opponent),
                provider_statuses=provider_statuses,
            )
            retry_raw = _clean_text(retry_data.get("speech") or _three_turn_repair_speech("B", cfg, turns, turn_no, b_latest_opponent))
            retry_text = _sanitize_fighter_speech(retry_raw)
            ...
            if not b_validation.get("three_turn_contract_pass"):
                b_text = _three_turn_repair_speech("B", cfg, turns, turn_no, b_latest_opponent)
        ...
        if cfg.turn_count == 3 and parity_failures.get("A"):
            ...
            if repaired_validation.get("three_turn_contract_pass") and repaired_validation.get("density_score", 0) >= a_validation.get("density_score", 0):
                ...
                a_meta.update(a_debug_trace)
        if cfg.turn_count == 3 and parity_failures.get("B"):
            repaired = _three_turn_repair_speech("B", cfg, turns, turn_no, b_latest_opponent)
        ...
        phase_entries.append(_phase_entry("postprocess", b_postprocess_started_at, turn=turn_no, speaker="B"))
    ...
    phase_entries.append(_phase_entry("judge_summary", judge_started_at))
    debate = {
        "topic": cfg.topic,
    ...
    phase_entries.append(_phase_entry("run_debate_total", run_started_at, completed_turns=len(turns)))
    return debate, provider_statuses, trace_entries, phase_entries
...
def _speaker_turn_data(
    ...
    if not key:
        ...
        return {**fallback, "_provider_mode": "mock"}
    try:
        if provider == "openai":
        ...
        provider_statuses[provider] = _provider_entry("mock-fallback", reason, raw_reason)
        return {**fallback, "_provider_mode": "mock-fallback"}
...
def _judge_summary_data(
    ...
    try:
        try:
            ...
            provider_statuses["judge"].update(
                {
                    "judge_provider": "gemini",
...
def _speaker_prompt(
    ...
    return (
        ...
        "- Schema: {\"speech\":\"...\",\"move\":\"opening|rebuttal|rally|finish\",\"meta\":{\"phase\":\"opening|rebuttal|rally\",\"target_issue\":\"...\",\"attacked_weakness\":\"...\",\"new_issue\":\"...\",\"collapse_signal\":\"...\",\"finish_intent\":\"push|finish|extend\",\"end_match\":\"yes|no\",\"opening_contract\":{\"claim_scope\":\"...\",\"comparison_axis\":\"...\",\"acceptance_condition\":\"...\",\"anti_reframe_guard\":\"...\",\"exception_policy\":\"...\",\"burden_target\":\"...\"}}}\n"
        "- meta is hidden scaffold only. Do not surface labels like target_issue, attacked_weakness, or any debate-notes wording inside speech.\n"
        ...
        "- In every turn, keep at least one concrete topic term from the proposition or positions visible in the speech. Avoid drifting into generic phrases like 'このテーマ' or 'この提案' alone.\n"
        ...
        f"{_speaker_side_role_rules(speaker)}"
...
def _three_turn_prompt_block(cfg: DebateConfig, turn_no: int) -> str:
    ...
    if turn_no == 1:
        base = (
            "- This is 3-turn mode: compression is not enough; density is required.\n"
            "- Turn 1 must include: clear stance, comparison axis, acceptance condition, and at least one concrete reason/example.\n"
            "- Do not end Turn 1 with only battlefield setup or a slogan.\n"
        )
        if dignity_topic:
            base += (
    ...
    if turn_no == 2:
        base = (
            "- This is 3-turn mode: Turn 2 must do two jobs at once.\n"
            "- Directly rebut the opponent, restate why your side still stands, and add at least one concrete noun/example.\n"
            "- Do not spend the turn only re-announcing the comparison axis.\n"
        )
        if dignity_topic:
            base += (
        ...
        return base
    base = (
        "- This is 3-turn mode: Turn 3 is final closing, not a thin summary.\n"
        "- Explain why your side remains, why the opponent collapses, and end with one short closing punch.\n"
        "- Do not end with a neutral wrap-up.\n"
    )
    if dignity_topic:
        base += (
...
def _build_proposition_lock(cfg: DebateConfig) -> dict[str, Any]:
    ...
    comparison_unit = "命題そのもの"
    evaluation_axis = _issue_pool(cfg.topic, "A")["reaction_terms"][0] if _issue_pool(cfg.topic, "A")["reaction_terms"] else "評価基準"
    ...
    proof_burden_shape = "賛成側は命題が一般に成立する採用条件を示し、反対側はその必要条件の不成立を示す"
    forbidden_reframes = ["条件だけへ逃げる", "例外だけで一般命題を上書きする", "時間軸をずらす", "比較軸を変える"]
...
def _opening_contract_prompt_block(cfg: DebateConfig, speaker: str, turn_no: int) -> str:
    ...
    return (
        "- For A Turn 1, lock an opening contract before the main push.\n"
        "- In natural Japanese, first make the comparison axis explicit, then the acceptance condition, then one anti-reframe guard, and only then expand the claim.\n"
        "- Prefer a narrower but defensible proposition over a broad proposition that needs rescue later.\n"
        "- Do not leave hidden conditions to be added in Turn 3.\n"
        f"- Comparison axis to lock: {contract['comparison_axis']}\n"
        f"- Acceptance condition to lock: {contract['acceptance_condition']}\n"
        f"- Anti-reframe guard to lock: {contract['anti_reframe_guard']}\n"
        f"- Exception policy to keep in mind: {contract['exception_policy']}\n"
        f"- Burden target to keep in mind: {contract['burden_target']}\n"
...
def _mode_prompt_rules(mode: str) -> str:
    ...
    return (
        ...
        "- Keep the speech natural and conversational. Do not use debate-label phrases like '相手の核心', '弱点は', '話をずらしてる', or 'それは苦しい' as surface wording.\n"
        "- If you need that logic internally, express it as natural argument instead of a visible label.\n"
...
def _speaker_side_role_rules(speaker: str) -> str:
    if speaker == "A":
        return (
            "- Side A role: assertive opener.\n"
            "- Cut to the claim early and narrow the battlefield fast.\n"
            "- In Turn 1, put the conclusion near the front instead of circling around it.\n"
            "- In Turn 2, press the opponent's weak premise and reduce their escape routes.\n"
            "- Prefer decisive framing, sharp compression, and direct pressure over cautious qualification.\n"
        )
    return (
        "- Side B role: skeptical checker.\n"
        "- Treat Side A's claim as something that must clear missing conditions, exceptions, and alternative readings.\n"
        "- In Turn 1, surface the condition that A is skipping or the rival interpretation A is ignoring.\n"
        "- In Turn 2, stop A's jump by saying what still does not follow and, if useful, raise one counterexample-first challenge.\n"
        "- Prefer condition-check, exception pressure, and narrowing what can actually be concluded.\n"
    )
...
def _axis_role_text(reframe_type: str, opening_debug: dict[str, Any], fairness_debug: dict[str, Any]) -> str:
    if opening_debug.get("reframe_detected"):
        return _axis_label_from_reframe_type(_clean_text(opening_debug.get("reframe_type") or ""))
    if opening_debug.get("drift_from_opening_contract"):
        return "Contract drift"
    if fairness_debug.get("burden_shift_detected") in {"A", "B"}:
        return "Burden shift"
    if fairness_debug.get("frame_survival") in {"A_frame_survived", "B_frame_survived"}:
        return "Frame survival"
    if fairness_debug.get("parasitic_rebuttal"):
        return "Parasitic rebuttal"
    return ""
...
def _expand_quote_to_display_excerpt(
    quote: str,
    turns: list[dict[str, Any]],
    turn_no: int,
    speaker: str,
    *hints: str,
    ...
    if speech:
        sentences = _split_quote_candidates(speech)
        normalized_quote = _normalize_quote_text(cleaned)
        if normalized_quote:
            for index, sentence in enumerate(sentences):
                normalized_sentence = _normalize_quote_text(sentence)
                if normalized_sentence and (normalized_quote in normalized_sentence or normalized_sentence in normalized_quote):
                    if _is_self_contained_quote(sentence):
                        return sentence
                    if index + 1 < len(sentences):
                        joined = f"{sentence}{sentences[index + 1]}"
                        if _is_self_contained_quote(joined):
                            return joined
                    if index > 0:
                        joined = f"{sentences[index - 1]}{sentence}"
                        if _is_self_contained_quote(joined):
                            return joined
        extracted = _ensure_self_contained_quote(cleaned, turns, turn_no, speaker, *hints)
        if extracted:
            return extracted
    if cleaned:
        clipped = _build_complete_quote_sentence(cleaned.strip("「」"))
        if _is_self_contained_quote(clipped):
            return clipped
    rebuilt, _ = _extract_transcript_quote(turns, turn_no, speaker, cleaned, *hints)
    if rebuilt and _is_self_contained_quote(rebuilt):
        return rebuilt
    return ""
...
def _mock_rebuttal_lead(plan: dict[str, str]) -> str:
    if plan["target_issue"]:
        options = [
            f"相手は{plan['target_issue']}だけで話を決めようとしてきた。",
            f"相手は{plan['target_issue']}を決め手にしたいんだろう。",
            f"いま相手が頼っているのは{plan['target_issue']}だけだ。",
        ]
        return options[0]
    return "相手は自分に有利な見え方だけで、そのまま話を決めようとしてきた。"
...
def _has_self_stance_reference(cfg: DebateConfig, speaker: str, speech: str) -> bool:
    ...
    cleaned = _clean_text(speech)
    return any(term in cleaned for term in own_terms)
...
def _three_turn_repair_speech(
    ...
    own_line = cfg.side_a if speaker == "A" else cfg.side_b
    if turn_no == 1 and speaker == "A":
        grounded = _topic_grounding_report(cfg, _three_turn_opening_surface(speaker, own_line, proposition_lock))
        if grounded.get("banned_template_phrase_count") or (
            _clean_text(cfg.topic or "").lower().find("sora") >= 0 and grounded.get("grounded_keyword_count", 0) < 4
        ):
            lock_type = _clean_text(proposition_lock.get("means_vs_essence_lock") or "")
            if lock_type == "product_sprawl_not_core_advantage":
                return _sanitize_fighter_speech(
                    f"SORAの撤退報道が本当なら、GPTが動画サービスに踏み込む判断は誤りだった。SORA級のモデルがあっても、動画事業は配信インフラ、権利処理、作品供給、継続投資を回せるかで勝負が決まる。OpenAIの強みは生成モデルと開発者基盤であって、総合動画サービスの運営まで抱えると収益化より固定費の方が先に膨らみやすい。撤退が示すのは単発失敗ではなく、技術優位だけでは動画サービスを支えられないという戦略判断の誤りだ。だから{own_line}"
                )
    if turn_no == 1:
        return _three_turn_opening_surface(speaker, own_line, proposition_lock)
    if turn_no == 2:
        return _three_turn_rebuttal_surface(speaker, own_line, proposition_lock)
    return _three_turn_closing_surface(speaker, own_line, proposition_lock)
