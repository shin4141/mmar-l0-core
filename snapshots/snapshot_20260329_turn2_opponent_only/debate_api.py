...
def _turn2_latest_only_strike(latest_opponent: str, weakness_line: str = "", fatal_line: str = "") -> str:
    ...
    opponent_first = _first_sentence(opponent_line).rstrip("。")
    issue = opponent_first or "その押し方"
    weakness = _clean_text(weakness_line or "")
    fatal = _clean_text(fatal_line or "")
    if not weakness:
        weakness = f"{issue}は前提を広げすぎていて、決め手になっていない。"
    if not fatal:
        fatal = "そこが閉じない限り、その反論は押し切りにならない。"
...
def _three_turn_repair_speech(
    ...
    if (
        ...
        if "金" in topic and "銀" in topic:
            return _turn2_latest_only_strike(
                latest_opponent,
                "中央銀行の準備資産運用では、流動性と担保実務が欠けると長期保有の優位は固定できない。",
                "そこを飛ばしたままでは、銀優位の主張は決め手にならない。",
            )
        return _turn2_latest_only_strike(
            latest_opponent,
            f"{concrete_a}や{concrete_b}のような具体例では、条件がずれれば同じ結論はそのまま通らない。",
            "その飛び方のままでは、一般命題まで押し切れない。",
        )
