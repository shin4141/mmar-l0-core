...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path not in {"/api/debate", "/api/debate_pure", "/api/ask_match", "/api/history/save", "/api/provider_preflight", "/api/judge"} and not path.startswith("/api/history/view/") and not path.startswith("/api/history/like/"):
            self._send_json(404, {"ok": False, "error": "not found"})
            return
        ...
        try:
            ...
            if path == "/api/judge":
                ...
                return
            if path in {"/api/debate", "/api/debate_pure"}:
                ...
                try:
                    result = run_debate_pure(request_payload) if path == "/api/debate_pure" else run_debate(request_payload)
                except Exception as exc:
