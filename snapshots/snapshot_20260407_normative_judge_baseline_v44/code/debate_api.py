...
def _run_debate_with_provider_fallbacks(cfg: DebateConfig, session: _SessionRuntime) -> tuple[dict[str, Any], str, list[dict[str, Any]]]:
    ...
    for turn_no in range(1, cfg.turn_count + 1):
        ...
        if (
            ...
            b_text = _sanitize_fighter_speech(
                "最後まで残るのは、SORA一件では参入失敗まで言い切れないという点だ。"
                " 相手は撤退を出したが、実装の失敗と参入判断そのものはまだ分け切れていない。"
                f" だから{_clean_text(cfg.side_b) or 'こちらの立場は残る。'}"
            )
            b_validation = _three_turn_validation_report("B", cfg, turns, turn_no, b_text, b_latest_opponent)
            b_debug_trace["repair_triggered"] = True
            b_debug_trace["repair_text"] = b_text
            b_debug_trace["repair_validation"] = dict(b_validation)
            b_debug_trace["adopted_stage"] = "topic-closing-override"
...
def _turn1_answer_prompt(topic: str, position: str, seed: str = "") -> str:
    ...
    clean_seed = _clean_text(seed or "")
    seed_block = f"First sentence seed: {clean_seed}\n" if clean_seed else ""
    return (
        f"Topic: {clean_topic}\n"
        f"{_debate_axis_line(clean_topic)}\n"
        f"Position: {clean_position}\n"
        f"{seed_block}"
        "\n"
        "Write the strongest possible argument for this position.\n"
        "\n"
        "Rules:\n"
        "- Start with a direct, strong conclusion.\n"
        "- The first sentence must be a plain assertion, not a question.\n"
        "- Provide exactly one causal chain.\n"
        "- Provide exactly one concrete example.\n"
        "- Add one short preemption if natural.\n"
        "- Do not hedge.\n"
        "- Do not balance.\n"
        "- Do not consider any opponent.\n"
        "- Do not explain debate structure.\n"
        "- Do not use words like 一方で, ただし, or 〜もある.\n"
        "- Do not mention any opponent, transcript, history, or debate meta.\n"
        "\n"
...
def _apply_scorecard_draw_tiebreak(
    ...
    payload["winner"] = {
        "side": favored_side,
        "reason": "scorecard Draw を blind explanation の成立条件破壊で解消した",
    }
    return payload
...
def _three_turn_repair_speech(
    ...
    if cfg.turn_count == 3 and turn_no >= 3:
        ...
        if speaker == "A" and "SORA" in topic and "動画サービス" in topic:
            return _sanitize_fighter_speech(
                "最後に残るのは、SORA撤退が単発ではなく事業の重さを露出したという点だ。"
                " 相手はまだ参入余地を言うが、運営負担と収益化の壁を越える筋を出せていない。"
                f" だから{own_line}"
