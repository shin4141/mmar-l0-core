...
def _speaker_prompt(
    ...
    proposition_lock_block = _proposition_lock_prompt_block(cfg) if turn_no != 1 else ""
    opening_contract_block = _opening_contract_prompt_block(cfg, speaker, turn_no) if turn_no != 1 else ""
    ...
    turn_mode_block = (
        ...
        "- Capture the opponent core in one short line, hit one weak point, and push your side.\n"
    ...
    goal_block = (
        ...
        if turn_no == 1
        else "Goal: win the exchange by attacking the opponent's latest core and keeping your own line standing.\n"
    )
    interaction_rules = (
        ""
        if turn_no == 1
    ...
    startup_rule = (
        ...
        if turn_no == 1
        else "- If opponent logic collapses, attempt to finish the debate.\n"
    )
    transcript_block = (
        ""
        if turn_no == 1
